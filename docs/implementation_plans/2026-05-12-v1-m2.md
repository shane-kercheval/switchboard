...
4. **Stream parsing.** Read stdout line-by-line; parse each JSON line; map per the table:
   ...
   | `{type: "item.started", item: {type: "command_execution", ...}}` | Emit `ToolStarted` with `name: "command_execution"`. |
...
5. **`TurnEnd` enrichment timing.** The session-file read happens after the terminal stream event arrives; enrichment writes back to the about-to-be-emitted TurnEnd's `usage.context_window`. **Delay TurnEnd emission until enrichment completes** (small added latency, single canonical event). On failure or timeout, emit TurnEnd without `context_window` rather than blocking forever.
   ...
   **Timeout + retry shape:** use a short timeout (suggested initial value: 2s) with a single retry (suggested 500ms backoff). If both attempts fail, emit `TurnEnd` without `usage.context_window` and `tracing::warn!`. The constants may be tuned during implementation if observed file-write latencies suggest different values — document the chosen values in the M2.4 commit and append to `docs/research/codex-cli-observed.md` findings if they differ from the suggested defaults.
...
- **Enrichment retry-then-success:** simulate a slow write — file absent on first attempt, present on the retry → `usage.context_window` populated, no warning logged.
- **Enrichment persistent failure:** file remains absent through both attempts → TurnEnd fires with `context_window: None`; `tracing::warn!` logged.
...
4. **Per-agent overview sidebar.** Lists the active project's agents. For each agent, render derived values:
   ...
   - **Transcript-derived** (computed at render time from `transcripts[id]`):
     ...
     - Context utilization bar: from `transcripts[id].filter(t => t.role === "agent").at(-1)?.usage` — compute `input_tokens / context_window` * 100, gated on both being present.
...
- **Concurrent in-flight turns (M2 ships this):** dispatch to agent A; while A is mid-stream (status: streaming), dispatch to agent B; assert both turns appear in the unified view simultaneously, both streaming, attributed correctly, no cross-talk in chunks.
...
- **Heartbeat reset on tool events:** inject `TurnStart` for a turn; wait 30s with no events; inject `tool_started` → heartbeat resets. Wait another 50s; inject `tool_completed` → heartbeat resets again. Wait another 50s with no events → at 60s past the last event, heartbeat fires, turn → failed. Codifies that all per-turn events (not just `content_chunk`) keep the turn alive.
...
- **Attach-time hydration** (paired with M2.6): on `attach_agent` success, `load_transcript(new_agent_id)` fires and the prior history appears in the unified transcript before the user sends any new message.
...
9. **Attach existing Claude session (with hydration)** — in a separate terminal: `claude -p --session-id $(uuidgen | tr A-Z a-z) --dangerously-skip-permissions 'remember the word PURPLE'` from the same working directory. Note the UUID. In Switchboard, "Attach existing session" → paste UUID → name `attached-claude` → submit. The unified transcript immediately shows the prior conversation history (the "remember the word PURPLE" exchange) — hydration runs on attach, not just on project reopen. Send "what word did I tell you to remember?" → reply references PURPLE.
10. **Attach existing Codex session (with hydration)** — similar flow with `codex exec ... 'remember the word ORANGE'`; capture `thread_id` from stream; attach. Prior conversation appears in the transcript immediately. Send a follow-up referencing ORANGE → reply recalls it. Sidecar file at `<directory>/.switchboard/projects/<project-id>/sessions/<attached-codex-id>.jsonl` has one line with today's `original_start_date_utc`.
...
8. **Hydration timing.** Hydration runs in three cases:
   ...
   The unified transcript view shows a skeleton / "loading transcript history..." placeholder per agent until its hydration resolves. New dispatches during load arrive on top via the existing reducer paths (the per-turn precedence rule from step 7 covers any race).
...
1. **Test layout.** Flat files directly under `crates/harness/tests/`:
   - `tests/claude_text.rs`, `tests/claude_tool.rs`, `tests/claude_error.rs`, `tests/claude_session_meta.rs`, `tests/claude_rate_limit.rs`, `tests/claude_transcript_load.rs`.
   - Same for Codex (`tests/codex_text.rs`, etc.).
   - `tests/dispatcher_smoke.rs` — single TurnStart-emission test.
...
- **M1 backend abstractions are stable; extend, don't redesign.** Changes to `HarnessAdapter`, `AdapterEvent`, the dispatcher, the `EventEmitter` trait, or the M1 registry layout — stop and ask. Frontend ownership reshuffles in M2.5 by design; that's expected. The "extend, don't redesign" rule scopes to backend.
