...
- Multi-pane layout: every agent's pane visible simultaneously, **each rendering live streaming output in real time** when its agent is mid-turn (e.g., one Claude Code agent and one Codex agent can both be streaming responses on screen at the same time, with the streamed text appearing in their respective panes concurrently). One focused agent (active for compose-bar input); collapse/expand background agents to manage screen space. M2's backend already supports concurrent multi-agent dispatch — M3 is purely the frontend work to render multiple live streams simultaneously.
...
- **Harness billing-awareness note in onboarding.** Switchboard spawns `claude -p` and `codex exec` — the two vendors meter headless / programmatic usage on fundamentally different models, and Switchboard's multi-agent fan-out pattern (especially M5 workflows) amplifies whichever model the user is on. Onboarding must explain both:
  ...
  - M7 scope is the **onboarding explanation only** (which pool gets consumed, link to vendor pricing). Whether to surface live cost/credit/quota in the agent UI is a separate **M2 design decision** — see `2026-05-12-v1-m2.md` "Open design decision — per-agent cost / credit / quota surface."
