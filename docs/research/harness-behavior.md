...
- ✅ **G4 — Claude has no auth pre-flight.** **Closed by reframing.** No auth pre-flight is the *intended* working-UI behavior for every harness now — auth is reactive-only. A presence-only Claude keychain probe lands in the getting-started milestone (no-project state), not as a working-UI banner.
