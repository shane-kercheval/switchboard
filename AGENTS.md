...
- **Codex post-terminal enrichment.** Codex's stream-json output omits `model_context_window`, `rate_limits`, `cli_version`, and `model`. The Codex adapter reads the session file once at each terminal stream event (`turn.completed` / `turn.failed`), extracts those fields, and emits in order: enriched `TurnEnd` → `RateLimitEvent` (if rate-limit info found) → `SessionMeta` (first turn only). The adapter holds the per-agent stream open past the terminal event until enrichment completes — closing immediately would silently drop the agent-scoped events. Session-file path uses the sidecar's `original_start_date_utc` (never `Utc::today()`). Enrichment failure degrades to a plain `TurnEnd` with `usage.context_window: None`; never propagates. See M2.4 (`docs/implementation_plans/2026-05-12-v1-m2.md`).
