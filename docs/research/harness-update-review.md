...
|---|---|---|---|
| Codex | `0.141.0` | 2026-06-18 | changelog + code review (`0.139.0`→`0.141.0`; notifier flagged `0.140.0` but `0.141.0` was already installed) — no impact. `exec` flags, stream vocabulary (`thread.started`/`item.*`/`turn.completed`/`turn.failed`), `turn_context` fields (model/effort/turn_id), session-file path (`~/.codex/sessions/<Y>/<M>/<D>/rollout-*-<uuid>.jsonl`), `401 Unauthorized` auth classification, and exit-code semantics all unchanged; parser stays skip-by-default (no `deny_unknown_fields`), tolerant of the additive rollout fields (compact-window id #27264, remote-compaction-v2 #27573) and new stream items (plaintext agent messages #27830, context-window/remaining tools #27488/#27518). **Encrypted-secret CLI auth** (#27504/#27535/#27539, Bedrock #27443) is opt-in/configurable and did **not** displace the default — `~/.codex/auth.json` still present at 0.141; the presence probe is display-only/best-effort regardless. `#26734` (Ctrl-C for non-TTY *unified exec*) and exec-server PathUri/cwd (#28032/#28122) touch the agent's tool-execution server, not the `codex exec` headless entrypoint we spawn. SQLite WAL pin (#27992) is the state DB, not our rollout JSONL. MAv2 / app-server / plugin / realtime / Windows changes don't touch the exec contract |
| Claude Code | `2.1.172` | 2026-06-11 | changelog + code review (`2.1.170`→`2.1.172`; `2.1.171` never published) — no impact. Two items touched our surface, both benign: (1) **nested sub-agents** (sub-agents spawning sub-agents, 5 levels deep) — our `parent_tool_use_id` filter skips on *any* non-null value (depth-agnostic), and sub-agent internals stay in unread `subagents/agent-<id>.jsonl`; the disk regression guard (`delegation_session_file_renders_as_single_parent_tool_call`) and `live_claude_subagent_collapses_to_parent_tool_call` both still hold. (2) **model-id `[1m]` / doubled-`[1M][1m]` suffix fixes** — `select_context_window` falls back to the max-`inputTokens` `modelUsage` entry when `result.model` doesn't key-match, so a suffix asymmetry still resolves the window; Switchboard never sets `ANTHROPIC_DEFAULT_OPUS_MODEL` so can't trigger the doubled-suffix bug, and model ids are opaque display strings. All other items are TUI / Bedrock / Windows / plugin / perf. Invocation flags, stream-json vocabulary, `result`/error shape, and session-file fields otherwise unchanged; parser stays skip-by-default. Prior auth-failure/overage probes still current |
| Gemini | `0.42.0` | 2026-05-27 | core probes from M3; logged-out auth shape captured 2026-05-27 |
| Antigravity (`agy`) | `1.0.10` | 2026-06-20 | re-probe (`1.0.2`→`1.0.10`, changelog + hands-on) — **no contract impact**. `transcript.jsonl` at `brain/<uuid>/.system_generated/logs/` is still the parseable source with a terminal `PLANNER_RESPONSE` (encrypted store migrated `.pb`→`.db` SQLite at 1.0.4, but we never read it); the flags we pass (`-p`/`--add-dir`/`--conversation`/`--dangerously-skip-permissions`/`--log-file`) all still exist and mean the same; new `--model`/`models` (1.0.5) we don't use. Timeout shape unchanged (`Error: timed out waiting for response` on stdout, exit 0 → `first_error_line` → `HarnessError`; forced-probe-verified). Two **new** SYSTEM transcript records (`EPHEMERAL_MESSAGE`, `CHECKPOINT`) are tolerated — `TranscriptRecord` is skip-by-default and both are `source≠MODEL`, so they emit nothing (same as `SYSTEM_MESSAGE`). 1.0.9 **fixed** the print-mode resume full-transcript replay — benign for us (the adapter sources content from the transcript, not stdout, so the old replay was already inert). **Pre-existing gap (not a bump regression) flagged for follow-up:** the adapter never passes `--print-timeout`, so it inherits `agy`'s 5-minute default. **Probe-confirmed (2026-06-20) that `--print-timeout` is a whole-turn wall-clock ceiling, not an initial-response-wait** — a turn that produced its first response in ~3s and was actively making progress was still cut at the deadline (`Error: timed out waiting for response`, exit 0, no terminal answer). `agy` is the only one of the four CLIs with such a built-in headless cap (Claude/Codex/Gemini expose no timeout flag and self-terminate on their completion signal). 1.0.7 raised the tool-call ceiling to 512, so long agentic turns (e.g. a PR review that runs a test suite) hit the 5m cap more often — the likely cause of a user "didn't hear back" report. Fix is Antigravity-only: pass a generous explicit `--print-timeout` in `build_args` so the adapter stops contradicting the dispatcher's intentional no-turn-deadline posture. Auth-failure shape **not** re-probed (needs desktop sign-out); 1.0.10's auth change is Windows-only (`rundll32`). |
