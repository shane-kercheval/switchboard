...
- **Multi-project backend shape already exists**: `AppState.projects`, `active_project_id`, `open_project`/`set_active_project`/`list_projects`, and the four harness adapters are all wired.
...
6. **Switchboard owns the user's side of the conversation; harnesses own the agents' side.** Switchboard persists an append-only **conversation journal** per project, with two record types:
   ...
   - a *turn-outcome* record on terminal **for cancelled turns only** (reliably absent from harness session files): `{ send_id, turn_id, agent_id, outcome: Cancelled{source}, started_at, ended_at }`, **no agent content**. Failed turns are *not* journaled — they render from the harness session file where the harness recorded them; failures the harness omits entirely (e.g. pre-stream/spawn) are absent from post-restart history (a documented, low-stakes M4 limitation). This avoids a fragile journal↔harness dedup with no shared key.

   The unified transcript is rendered as: **user turns from the journal** (grouped by `send_id` → one message attributed to the union of recipients, e.g. "User → B|C"; the group's render timestamp is the `min(at)` across its records) **+ agent response turns from harness session files** (assistant-role content only; harness *user-role* entries filtered out of the unified view) **+ cancelled markers from the journal**. After restart a partially-queued fan-out shows **only the recipients whose turns started or were cancelled-after-start** — e.g. "User → A" if B was still queued when the app closed (B's turn never ran; this is intended and more truthful than implying B received it, *not* a hydration bug; it is a visible difference from the live UI and must be documented as such). This **refines** the earlier "harness files are the sole source of truth / Switchboard keeps no transcript store" invariant — which a review showed cannot represent the user's side: a fan-out duplicates the user prompt across N harness files, and an instantly-cancelled send appears in none. So: harness files own **agent-produced content**; Switchboard owns the **user-send journal + cancelled-turn outcomes**. Buffered partial *content* of a cancelled turn stays in-memory only (lost on restart). See system-design §3/§7. The journal is the persistence shape M6 extends for workflow/step outcomes.
...
8. **Directory-wide session-id uniqueness (already enforced on the attach path; M4 verifies/preserves):** an agent (harness + session-id) may not be registered in more than one project in the same working directory — same-session parallel invocation is the hazard (`docs/research/same-session-parallel-invocation.md`). The attach path already scans all on-disk projects; create-new can't collide (fresh/server-assigned id).
...
- The user's send and cancelled-turn outcomes are persisted in a Switchboard-owned conversation journal, so after restart the transcript shows the user's message once (even for a fan-out) and shows cancelled turns as such (failed turns render from the harness file); partial streamed *content* of a cancelled turn is not persisted.
- In-flight turns are cancelled (not orphaned) on project unload and app shutdown, tagged with the right `source`, and the project lock is released only after they drain.
...
- *outcome* — written on terminal **for cancelled turns only** (reliably absent from harness session files): `{ type: outcome, send_id, turn_id, agent_id, outcome: Cancelled{source}, started_at, ended_at }`, **no agent content**. Failed turns are *not* journaled — they render from the harness file where the harness wrote them; harness-omitted failures (pre-stream/spawn, no session-file entry) are absent from post-restart history (a documented, low-stakes M4 limitation — this removes a fragile journal↔harness dedup that would have no shared key). Completed turns write no outcome record (content is in the harness file).

The journal is Switchboard's user-side source of truth; agent content is never written here. Document in a comment that this refines the "Switchboard stores no transcript" invariant: Switchboard owns the user-send journal + cancelled-turn outcomes, harness files own agent content. Hydration/merge is M4.6.
...
**Lifecycle wiring (also resolves review Topic 4 — lock-release ordering).** On project unload/rebind and on app shutdown, iterate the affected agents via the `AppState` register-cache (M4.1) and call `dispatcher.cancel(agent_id, Shutdown)` for each in-flight turn, **await drain, then release that project's lock** — never release the lock while a turn is still live (that would reopen the double-drive race the lock prevents). This resolves the M1.4 "orphaned drain tasks on rebind/shutdown" deferral. The drain task still emits `AgentIdle` last on the cancel path.
...
- **Unit/integration tests (fixture-driven, no live harness):**
  ...
  - A send record is written at turn-start (with `TurnStart`); a queued message removed before it starts is never journaled (absent after restart); an idle send then instant cancel keeps its record. Outcome records are written for cancelled turns only; failed turns are not journaled; completed turns write no outcome record.
  - Re-hydration renders the user's message once (grouped by `send_id`) and a cancelled turn as a marker (full merge tested in M4.6).
  - Project unload and app shutdown cancel in-flight turns with the correct `source` and the project lock is released only after they drain.
...
Make multiple projects loadable in one session with a switcher, where switching is display-only and background projects keep running.

Outcomes:
- The loaded app always knows: the bound directory, the list of projects in it, the active project, and the agents per project.
- The user can switch which project's transcript + sidebar is displayed; switching away does not stop the other project's agents or tear down their event subscriptions.
...
- App startup does not parse every project's full transcript history (eager registry, lazy hydration).
- After restart, each of the user's messages appears once (grouped by `send_id` across the recipients of a fan-out), cancelled turns appear as markers (failed turns render from the harness file) — the unified view merges the conversation journal (user side) with harness session files (agent side).

### Implementation Outline

**State reshape (the core of this sub-milestone).** Today the frontend effectively represents one selected project at a time. Promote app state to hold, concurrently: `directory`, `projects` (list), `activeProjectId`, and an agents-by-project structure, with the existing per-agent `transcripts` / `runtimes` maps continuing to be keyed globally by `agent_id` (they already are — agents are globally unique, so no per-project nesting of transcript state is needed; only the *roster* is per-project). The unified transcript and sidebar render the active project by filtering the agent roster to `activeProjectId`, then reading the global per-agent maps. Confirm this against `src/lib/state/index.svelte.ts` before restructuring.
...
**Eager registry, lazy hydration.** On directory bind / startup, load all projects and their agent rosters (cheap — `list_projects` + `list_agents`, backed by the M4.1 register-cache). Defer transcript hydration (the expensive session-file parse) until the first time a project becomes active; cache the hydrated state and don't re-hydrate on subsequent switches. Show a loading indicator during first hydration. Background agents stream regardless of hydration (live events append to the per-agent transcript; hydration backfills history).

**Hydration merge — the conversation journal (decision 6, review Topic 3).** Hydrating a project no longer takes user turns from harness session files, and it becomes **project-scoped**, not per-agent (review found the current `load_transcript(agent_id)` per-agent shape can't dedup fan-out user messages across agents). Add a project-level backend command — `load_project_conversation(project_id)` — returning a merged shape: grouped user sends, per-agent assistant turns, cancelled markers, parse warnings, and metadata. (If you keep the per-agent `load_transcript`, add a separate `load_conversation_journal(project_id)` and specify the exact frontend merge algorithm + tests — but the single project-scoped command is preferred.) The merge is three sources, ordered by timestamp (`started_at` for turns; for a user message, the `min(at)` of its `send_id` group):
...
- **agent response turns** ← harness session files, **assistant-role content only** — filter out the harness files' *user-role* entries from the unified view (they are per-agent context, not the canonical user record).
- **cancelled turns** ← the journal's *outcome* records (reliably absent from harness files). **Failed turns are not journaled** — they render from the harness file where the harness wrote them (no journal↔harness dedup needed; decision 5).
...
**Switcher UI.** A switcher in the sidebar header or top bar listing the directory's projects, marking the active one, with the in-flight/has-activity state visible enough that the user knows a background project is doing something. Single window only; no multi-window.
...
- **Component tests (mock `invoke` + `listen`, per the AGENTS.md frontend testing guidance):**
  ...
  - Eager load populates the project list and rosters without hydrating transcripts (assert hydration is not called at startup for non-active projects).
  - A background agent receiving a `content_chunk` / `turn_end` after a switch-away updates its state correctly.
  - Post-restart hydration of a project that had a fan-out send renders the user message **once** (grouped by `send_id`), renders a cancelled recipient as a marker at its `started_at`, and does not double-render harness-file user-role entries.
- **Manual verification (state explicitly if you cannot run the UI):** in `make dev`, open two projects, dispatch in one, switch to the other, confirm the first keeps streaming and is current on return.
