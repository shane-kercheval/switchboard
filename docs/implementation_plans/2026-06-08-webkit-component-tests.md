...
- Two items deferred from the M1 review, to resolve here once the multi-element specs make them concrete: (1) the `preview-clip` testid disambiguation — the multi-turn/fan-out specs are the first to mount more than one clip, so this is where to either scope locators to the owning turn or add type-specific testids (the M1 harness header documents both approaches); (2) the per-spec IPC mock block — browser-mode `vi.mock` hoisting forces it into each spec, so as the specs multiply, add a cheap guard against silent drift (e.g. a grep/lint check that every `*.browser.test.ts` declares the block, or a shared assertion that the mocked surface matches what the component imports).
