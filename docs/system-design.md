...
- **Authentication policing.** Switchboard is **auth-agnostic** at the application layer: it invokes each harness CLI and lets the CLI own its auth (interactive login, API key, Vertex, etc.). Switchboard does not enforce any particular auth method, does not store or inject credentials, and does not block agent creation on auth grounds. A logged-out harness is discovered reactively — the user sends, the turn fails in the transcript with an adapter-authored actionable message naming the recovery command (`claude login` / `codex login` / etc.) — and the user signs in and sends again. There is no proactive banner, no per-agent auth status icon, and no agent-creation gate. (The backend retains presence-probing commands for the getting-started surface — a no-project state shown when there's nothing else to look at — not for the working UI.)
