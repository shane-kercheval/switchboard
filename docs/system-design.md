...
Switchboard is a **human-directed orchestrator for AI coding agents** — a desktop application you run alongside your existing Claude Code, Codex, and Antigravity setup. It lets you spawn multiple agent sessions within a project, route messages between them, and define reusable workflows for common multi-agent operations like second-opinion code review, plan-and-implement, and parallel-solution adjudication.
...
This orchestration model has a useful side effect for prompt management. Because Switchboard resolves prompts itself and sends agents plain text, **prompt-provider configuration lives in *one place* (Switchboard) and works the same way across every agent backend**. A user's prompt library — whether in Tiddly, another MCP server, or Switchboard's local store — uses the same lookup and invocation surface across every harness, without configuring the prompt source in any of them. This is especially useful for harnesses whose native MCP prompt support is limited or absent. (The same does **not** hold for MCP tools or per-harness skills, which are invoked by the model mid-turn and must still be configured in the underlying agent; see section 6.)

## 2. Goals and non-goals

### Goals

- **Multi-agent spawn and management.** Multiple Claude Code, Codex, and Antigravity agent instances run in a single project with user-assigned names.
...
- **Agent-friendly authoring.** Workflow files and other authorable artifacts (local prompts, project setup) are documented in instruction docs under `docs/agent-instructions/` designed for AI coding agents to consume. The intended authoring path is to point an existing Claude Code, Codex, or Antigravity agent at the relevant instruction file and ask it to generate the artifact from a description, rather than learning the DSL by hand.
...
- **Replacing the Claude Code, Codex, or Antigravity harness.** Compaction, tool rendering, permission policy, plan mode, hooks, and skills all live in the harnesses. Switchboard drives them via their non-interactive modes (`claude -p`, `codex exec`, `agy -p`).
...
- **Claude harness slash commands through the ordinary message composer.** A plain send to a Claude agent means “send this text to the model,” including text beginning with `/`; it never implicitly invokes a Claude-local command. Switchboard-owned prompts use the `/` picker, while any future Claude-command execution requires an explicit, separately-labelled action. Other adapters retain their documented native CLI behavior.
- **Pricing tables / dollar normalization for pay-as-you-go users.** v1 does **not** ship per-model pricing tables to derive dollars from token counts. Dollar cost is surfaced where the harness itself reports it (subscription Claude's `total_cost_usd`) and quota / rate-limit signals are surfaced where harnesses report those (Codex answers a direct account query for every quota it meters); auth modes that don't carry these signals simply show no cost figure, which is correct — not an error. This is a load-bearing product decision: it shapes the cost / quota surface (see §7 "Per-agent status and actions") and removes the maintenance burden of keeping pricing tables current with vendor releases.
- **Authentication policing.** Switchboard is **auth-agnostic** at the application layer: it invokes each harness CLI and lets the CLI own its auth (interactive login, API key, Vertex, etc.). Switchboard does not enforce any particular auth method, does not store or inject credentials, and does not block agent creation on auth grounds. A logged-out harness is discovered reactively — the user sends, the turn fails in the transcript with an adapter-authored actionable message naming the **verified** recovery command (`claude auth login` — top-level `claude login` does not exist; `codex login`; run `agy` interactively, which has no login subcommand) — and the user signs in and sends again. There is no proactive banner, no per-agent auth status icon, and no agent-creation gate **during work**. Proactive install/auth status lives in exactly two **opt-in** places, both built on a shared `HarnessStatusList` (install-probe + best-effort auth-presence ✓/✗, re-probed on mount and on window refocus): the **no-project orientation surface** (a welcome page explaining projects + agents, with a "Supported CLIs" table) and a **Settings → "Supported CLIs"** section. Neither interrupts nor goes stale, so they don't reintroduce the removed mid-work banner. Vocabulary: user-facing copy says "tool"/"CLI" for the product and "agent" for an instance of one running in a project; "harness" is internal-only.
- **Raw token counts in the UI.** Token usage is plumbed through the normalized event stream (and used to compute context utilization) but is not surfaced as a UI affordance for users in v1. The user-facing cost / quota surface is dollars (Claude Code, via `total_cost_usd`) or rate-limit / quota signals (Codex, via the account quota read). See §7.
- **Cross-harness dollar normalization.** Switchboard does not synthesize a single "total spend" number across a mixed-harness project. Claude Code's dollars and Codex's rate-limit signals live in different billing realities (separate vendors, separate auth tiers, different metering models per §2 below); aggregating them into one number would mislead. Per-agent and per-project aggregates within a harness are architecturally permissible; cross-harness aggregation is not. (v1 does **not** surface a per-agent cost *total* on the agent card — that accumulating, resets-on-reopen figure read as a running total but wasn't one, so it was removed; per-turn cost renders inline per-message on real-spend turns instead. See §7.)
...
| **Harness session** | The underlying Claude Code, Codex, or Antigravity session that backs an agent. Persisted on disk by the harness; resumed via `--resume` (or `--conversation <uuid>` for Antigravity). |
...
> **Scoped exception — workflow run checkpoints (M6).** Workflow run checkpoints (`projects/<project-id>/runs/<run-id>.jsonl`) may store **bounded resolved outputs** — the text one workflow step forwards into a later step — needed to resume deterministic workflow execution after a crash. This is *not* the transcript store this invariant protects: it is per-run runtime data, never rendered as conversation history, scoped to text that entered a workflow's output scope (text-only, no thinking/tool output), and discarded with the run. It exists because a workflow's `forward_from`/fan-in must re-feed an earlier turn's output on retry, and the harness session file cannot be joined back to a specific dispatcher turn (the dispatcher `turn_id` and the harness file's turn ids are different id spaces — see §7 and the M6 plan). Capturing the resolved text at completion is the deterministic, harness-agnostic alternative to a fragile order-based disk re-derivation.

- **Switchboard owns the user's side** — a per-project append-only **conversation journal** (`projects/<project-id>/journal.jsonl`) recording each *send* (prompt + recipient + any **attachments** the user dropped on it + a `send_id` that groups a fan-out's recipients; written **when the turn starts**, one record per recipient) and an *outcome marker* for every **non-completed** turn — failed or cancelled — (outcome metadata only, including the failure reason; never agent content). Attachments are part of the user's side: the `Send` record stores the clean prompt plus a structured list of staged files (each a `label`, `kind`, absolute `path`, and original name), while the agent receives those paths appended to the prompt as a footer at dispatch time. The staged files live in the per-project `attachments/` dir (above) and persist as durable conversation content; a load-time reference-GC pass deletes any staged file no longer named by a `Send` record (orphans from a staged-but-unsent drop), and a fan-out's N records all reference the one shared staged file. The send record is written **fail-closed, immediately before the harness subprocess is spawned**: if it can't be persisted, the turn does not start (a silently-lost send would surface as an assistant reply with no user message above it). A turn that *fails to start at all* — the harness errored before any stream — gets a `Failed` outcome marker too, so it shows as a failed turn rather than an orphaned send. **Durable history begins when a turn starts, not when the user hits Send:** a message still queued behind a busy agent is live-UI-only and lost on restart (the queue is in-memory), and a queued message removed before it starts is never journaled. A third record — a **`TurnLink`** — is written at a **content-bearing** turn's terminal (completed **or** failed-with-content) carrying that turn's durable `hydration_key`; it is the send↔turn join key. This does **not** breach "never agent content": a `hydration_key` is a harness per-message *identifier* (normally Claude's first assistant `message.id`), not a response or tool call — the same category as the `message.id` the per-turn telemetry store already keys on. See §7 "Sends and turns."
...
Alongside the journal, Switchboard keeps a small **per-agent stream-only-metadata cache** (`projects/<project-id>/sessions/<agent-id>.meta.json`). It is not conversation content — it's a last-known snapshot of metadata that lives *only* on the live event stream with no harness-file equivalent (today: Claude's rate-limit / overage payload and context window), so it would otherwise vanish on restart. Written last-write-wins on each such event and re-read on project open; the rate-limit payload carries an "as of …" qualifier, while the context window is rejoined only to its exact final assistant message id and matching model. Harness-agnostic and keyed on `AgentId`; best-effort, not load-bearing: a missing or corrupt cache degrades to absent telemetry, never a failed load. A sibling **per-turn telemetry store** (`projects/<project-id>/sessions/<agent-id>.turnmeta.jsonl`) carries stream-only *per-turn* cost + overage attribution — an **append-log** (one record per real-spend turn, keyed on the harness per-message id), distinct in shape from the last-write-wins `.meta.json` snapshot — re-joined onto the matching hydrated turn on reopen so a past turn's inline cost / "using credits" marker survives restart (no backfill of pre-feature turns). Same best-effort posture. See `docs/harness-behavior.md` §3.1 (the event ⟂ on-disk parity classes), §3.1-cost (the per-turn join), and G13.

The two sources **partition** by turn outcome (a completed turn's content comes from the harness file; a failed or cancelled turn's marker comes from the journal — the harness file is not consulted for it), and are **correlated by a durable key-join where one exists, positional order otherwise**. Each content-bearing turn's `TurnLink` matches its harness turn to its send by `hydration_key`, so a stray/extra/missing harness record can no longer shift a *different* send's grouping — the failure mode of the old count-only correlation. Turns and harnesses with no such key (Antigravity, pre-feature history, in-flight turns) fall back to the positional path, which correlates by order and is where the residual positional edge-cases live. The unified transcript is the merge: user messages from the journal (grouped by `send_id`, rendered once — a fan-out is one "User → B|C" entry, not one per recipient), completed-turn content from the harness files, and failed/cancelled markers from the journal. One refinement for an **attached** session whose history predates journaling: those turns have no `Send`, so their user prompts are read from the harness file instead (the merge partitions at the agent's first journaled send — see §7 example 6). What shows after restart for a non-completed (failed or cancelled) turn is therefore exactly what the *harness* persisted: Switchboard adds no agent content of its own, and the merge renders whatever assistant content the harness session file holds, with the journal's outcome marker overlaid by timestamp. Claude Code and Codex persist **nothing** for an aborted turn (their session files stop at the last completed turn), so those show the marker only; a harness that *does* persist partial content would show it automatically, above its marker, with no special handling (Antigravity cancellation persistence is **unverified** — see the per-harness research notes). The live session always shows partial output from Switchboard's in-memory stream buffer, which is discarded on restart; "open session file" exposes the raw harness content. (Worked examples: §7 "Unified history after restart".) After restart a partially-queued fan-out shows only the recipients whose turns actually started (e.g. "User → A" if B was still queued) — intended, since B never received the message. A per-agent "what this agent saw" view can still read a single harness file verbatim (including the user message as that agent received it).
...
- Agent type (Claude Code, Codex, or Antigravity).
...
Authoring is intentionally file-based. The user edits workflows in whichever editor they prefer (Vim, VS Code, etc.); Switchboard's UI reads the files but does not include an editor of its own. The supported authoring path for new users is to point an existing Claude Code, Codex, or Antigravity agent at `docs/agent-instructions/workflows.md` and have it generate a starter workflow from a description (per §2 "Agent-friendly authoring"). Hand-authoring against the DSL spec works too for power users.
...
Switchboard resolves prompt IDs itself and sends agents the rendered text as a plain message — the agent never sees the MCP call, the provider, or the arguments. The useful side effect: prompt-provider configuration lives in *one place* (Switchboard) and uses the same lookup and invocation surface across every agent backend. A user's prompts (Tiddly, another MCP server, or the local store) work the same way with every agent, without configuring the prompt source in any harness. (The actual rendered text can differ — local prompts render via MiniJinja, MCP-served prompts render server-side; portability across providers is bounded by the shared template-feature subset captured in 6.1.) This is especially useful for harnesses whose native MCP prompt support is limited or absent — Switchboard gives them a Claude-Code-style prompt library experience without requiring native support.
...
A **per-agent overview sidebar** lists every agent in the project with its real-time operational state — name, harness badge, status (idle, processing, waiting on tool, errored), and context utilization. **Quota is not one of these per-agent fields.** It sits in a **Usage limits** section pinned to the bottom of that sidebar, below the roster, carrying one card per harness, because a quota belongs to the account the harness is logged into rather than to an agent or a project: per-agent cells published one account-level fact once per agent, each as stale as that agent's last turn. The section shows the newest reading whatever produced it, persists user-globally, and dates every reading in its tooltip. **The two harnesses reach it by different contracts, and collapsing them into one is how the bugs this replaced got written.**

- **Claude** is *reported to*. Its stream payload is a partial view of the account — the model-gated weekly cap is absent unless that turn ran on a gated model — so **each window is retained and ranked on its own** rather than as part of a whole reading; replacing the held set wholesale deleted a cap that was still in force. A window's warning tone comes from the delivering payload naming it as refused, **excluding turns that are being served on overage credits**, which carry the same refusal status while work still succeeds. **Nothing ever asks Claude anything**: there is no query for this, so its card is exactly as stale as the last Claude turn that streamed a reading, and opening the panel does not refresh it.
- **Codex** is *asked*. One account query returns every quota at once, so the reading is complete and replaces the held one whole. A quota's warning tone is our own inference from the measured percentage reaching 100%, not something Codex stated — it does send a reason code and we deliberately ignore it. The query runs after every turn whatever its outcome, and when the panel is opened — the panel is the only consumer of the reading, so it asks for its own refresh rather than being warmed at launch.

**Agent cards themselves default to collapsed**: the roster is a scannable list of who is in the project and what each one is doing, and the quota that was once the reason to keep a card open no longer lives there. Per-turn cost is **not** a sidebar field — it renders inline next to the message that incurred it, and only on turns with real, user-billed spend (see §2). Agent row actions expose session controls such as open session file, reset/remove, and cancel in-flight turn. (Fork is deliberately **not** a row action — a Claude fork can only materialize as a turn, so it lives in the compose bar as a send-time option; see §7.) The right sidebar switches between this agent-management surface and a project-scoped **Pins** reading surface. Each pin renders the complete user or agent message with the transcript's Markdown, reasoning, and tool-call components and can be collapsed independently; navigation back to the transcript is a secondary action. A user prompt is pinnable once; each recipient response in a fan-out is independently pinnable, while the fan-out aggregate is not. Locating a pin uses the transcript's normal find/jump path, including restoring the target pane and expanding compact content. Pins store only durable message identities and timestamps in `pins.jsonl`, never copied response content, so the harness transcript and conversation journal remain authoritative. A live send identity may migrate to its harness hydration identity only when the association comes from that same live turn or a durable `TurnLink`; positional transcript correlation is display-only and can never resolve or migrate a pin. The Agents and Pins modes remember separate widths.

Agent cards report operational and hydration health, including history that
could not be recovered. They do not aggregate best-effort transcript parser
warnings. A recoverable diagnostic appears only on the transcript item whose
degraded data the parser can identify; unowned salvage diagnostics remain
developer-facing rather than painting the whole agent as unhealthy.
...
Sometimes the user wants to *watch* a project run without being in a position to act on it — following a long fan-out, or reading back through a finished conversation. **Reading mode** is the per-project posture for that: it hides the compose box, and it asks Switchboard to treat the user as though they were not in the project at all.

That second half is the point. Completion notifications are normally skipped for the project on screen — the transcript is right there — so a user watching project A gets no alert when A finishes, which is precisely when they want one. With reading mode on, A's completion follows the same rule as any other project: it notifies if the user has enabled "also notify me about other projects while I'm using Switchboard". Reading mode does not override that preference; if it is off, Switchboard says so where the toggle is rather than quietly ignoring the setting.

Everything else keeps working: live streaming, panes, pinned messages, the sidebars, and — importantly — the workflow progress view, which occupies the compose slot and is often the very thing being watched.
...
- **Model**: the model identifier the harness reported on this dispatch (e.g. `claude-sonnet-4-5`, `gpt-5-codex`, `gemini-3.7-flash`).
...
- **Context utilization**: % of model context used, derived from the newest non-streaming usage-bearing agent turn's post-turn parent occupancy and that exact turn/model's reported context window. Whole-dispatch billing totals (including auxiliary/subagent work) are not context occupancy. A streaming turn leaves the last completed value visible; once terminal telemetry arrives, missing, turn/model-mismatched, overflowed, or numerically impossible data clean-hides rather than falling back to an older turn, guessing, or clamping. Surfaced as a progress bar so the user can see when an agent approaches the auto-compact threshold. Per-harness availability — see "Per-harness sidebar surface" below.
- **Rate-limit / quota signal**: harness-asymmetric per the v1 cost surface (§2). **Account-scoped, so it is not a per-agent cell**: it renders once per harness in the sidebar's Usage limits section pinned below the roster, from the newest reading any agent on that harness reported in any project. Claude contributes its window map plus a current overage status, Codex its account-wide quota percentages — **not** a per-turn dollar figure. Per-turn cost is a *per-message* surface in the transcript (shown only on real-spend turns), not a sidebar cell; there is no per-agent cost total on the card. Per-harness availability — see "Per-harness sidebar surface" below. **No raw token counts are surfaced in the UI for any harness** — the underlying tokens are plumbed through the normalized event stream and used to compute context utilization, but the user-facing surface is dollars (Claude, per-message) or quota (Codex), not "X input tokens." See §9 for the normalized event vocabulary.
...
| Status | ✓ | ✓ | ✓ |
| Model | ✓ | ✓ | ✓ |
| Overage status (current) | ✓ | — | — |
| Quota % (window used) | ✓ | ✓ | — |
| Context after last turn (%) | ✓ | ✓ | — |
| Environment inventory (MCP servers, skills, and the rest) | ✓ | ✓ (subset) | ✓ (names only) |
| Context breakdown (on demand) | ✓ | — | — |

**Per-asymmetry rationale**:

- **Overage status (current)** — only Anthropic's stream reports `isUsingOverage`, so the sidebar shows whether the agent is *currently* spending overage credits (Bucket-A "as of now" state). The per-turn dollar figure (`total_cost_usd`, drawn from the Agent SDK credit pool post-2026-06-15) is **not** a sidebar cell — it renders inline next to each message and only on real-spend (overage) turns, because subscription Claude's `total_cost_usd` is a notional API-equivalent figure, not money charged unless the agent is in overage; showing it on every turn would imply spend that didn't happen. Codex (subscription auth) surfaces no dollar number (OpenAI's subscription billing is quota-based, shown as Quota %); Antigravity surfaces no usage/cost/token data at all (it is a thin client to a server-side agent — all accounting is server-side and never written to disk).
- **Quota %** — both Claude and Codex report window usage, in different shapes. Codex's `RateLimitEvent` carries `primary` / `secondary` windows labeled from `window_minutes`; Claude's carries a `unifiedWindows` map, and every **recognized** key in it renders as its own meter (5-hour, weekly all-models, and a model-gated weekly cap). Keys outside that list are dropped rather than labelled by guesswork, so a window a future plan adds does not appear on its own. Claude's map is **undocumented**, found by probing rather than from any published contract, so a live test guards it; see [harness-behavior.md](harness-behavior.md) §3.12 for the key map. Antigravity exposes none.
- **Environment inventory** — one row rather than the separate MCP-count and skill-count cells this matrix used to list, because that is what ships: the counts survive only inside the row's collapsed summary line. It reports what the harness actually loaded, rather than how many things it has: MCP servers with live status and config scope, custom agents, plugins, memory paths, skills, tools, slash commands, and the run settings. Claude reports all of it in `system/init` (stream-only, so it is persisted and shown with an "as of" qualifier after a restart); Codex reports a subset from its rollout — skills with descriptions, the approved-command allowlist, and the run settings — and **no tool or MCP status exists anywhere**, so its MCP names come from config with no status dot. Every list collapses to a count line and expands; nothing the harness reports is withheld for being long.
- **Context breakdown** — what is *in* the context window, per category and per item, as opposed to how full it is. Claude only, on demand; see the per-agent action below.
- **Context after last turn (%)** — Claude binds the stream's `result.modelUsage.<model>.contextWindow` to the final parent assistant's exact resolved model id and divides the final parent call's post-turn occupancy by it; the usage-map key may add the observed `[1m]` context qualifier, but persisted provenance remains the assistant model used by the transcript. Codex enriches both operands from the session file's per-turn records. Antigravity's transcript carries no token/context data, and while its control stream's `result.usage` does report raw token counts, no **context window** appears anywhere — so it cannot compute the ratio. If upstream telemetry adds a context-window field, the asymmetry closes.

Antigravity's model cell populates from its `SessionMeta` (parsed from the user-settings envelope) and its Environment row from `~/.gemini/config/` — MCP server and skill **names** only, with no runtime status; its cost / quota / context cells clean-hide. Quota is not a ceiling — `agy -p "/usage"` returns one — but polling it costs seconds on every turn, so it was costed and declined (harness-behavior.md §5.1).
...
- **Fork** — create a new agent branched from the current state. Native in Claude Code via `--resume` + `--session-id` + `--fork-session` (the forked id is caller-controlled). Because a Claude branch can only come into existence *as a turn* (the CLI refuses a promptless fork — harness-behavior §3.5), Fork is a **send-time action on the send button itself** — its second half, a branch icon (`⇧⌘↵`) — not an agent-row action: with a single Claude agent selected, pressing it creates `<name>-fork`, places it in its own pane, moves the compose selection to it, and dispatches the message as the branch's first turn. There is no armed state; choosing to branch and sending are one action. Available in plain compose mode and with a saved prompt. A prompt fork renders **before** it branches — a fork is a registry append, so branching first and then failing the render would leave a visibly empty agent that received nothing — and nothing is cleared until the send has actually dispatched, so a refused fork costs the user nothing. Because that lifecycle spans two awaits (the render can open a browser for an MCP sign-in and wait minutes) it outlives its own compose bar, which is remounted on every project switch: single-flight, the outcome message, the "your composer was consumed" reconciliation, and whether recipient targeting is frozen are therefore all derived from one project-scoped operation claim rather than mirrored in component-local state. **A sign-in wait has no bound** — its callback and token exchange are timed out, but the credential commit behind them deliberately is not, so it can never report a failure it didn't have. The composer therefore offers **"Stop waiting"**, which releases *Switchboard's* compose operation only: it does not cancel credential persistence, which may still succeed, and every continuation re-checks that it still owns the claim after the sign-in wait — the only await abandonment is permitted during — so a late completion acts on nothing (past that point ownership is stable, and a registered branch's first message dispatches unconditionally). Do not "fix" this with a backend timeout. A composer that changed while the render was open **aborts** (nothing is committed yet, so dispatching text the user has since replaced would branch onto content they abandoned); one that changed after the branch was registered still dispatches — abandoning it would leave a promptless fork — and keeps the newer draft and recipients, saying so. **Not** available while composing a workflow (a workflow is N sends; which one branches has no answer) or with a prompt whose fields are filled from another agent (that composes server-side on an unbounded hold, so a branch would sit promptless or register against a long-stale gate). **Hidden**, not disabled, for non-Claude recipients, multi-recipient selections, an agent with no session yet, attached forward sources (Fork and Forward are both send modifiers and don't compose), a send already in flight, or a parent that is mid-turn (a mid-turn snapshot would inherit a synthesized "No response requested." stub in place of the parent's in-flight answer). **This reverses an earlier decision to keep it visible-but-disabled with an explanatory tooltip.** That reasoning held for a labelled chip, where the label is what teaches; as an unlabelled icon a dimmed half is unclickable noise that explains nothing. The explanation did not disappear with it — the keyboard shortcut stays live in every one of those states, and refuses with the reason rather than silently swallowing the keystroke, which is where a user who expected Fork actually finds out why they can't have it. The mid-turn block is re-checked authoritatively when the send dispatches, and that in-process look is now backed by a **cross-process advisory lock**: every turn holds one keyed on `(harness, session locator, cwd-namespace)`, and an unmaterialized fork additionally holds its **parent's** for the whole of its first turn — so a second Switchboard instance (a dev build alongside the installed app, which share the harness's session files even though their stores are separate) is refused rather than slipping through. Holding the parent's key makes the rule **symmetric**, which the in-process gate alone was not: a parent cannot start a turn while a branch is materializing from it, just as a branch cannot materialize while the parent is mid-turn. That costs the parent one turn's wait, bounded to the branch's *first* turn — after materialization the branch never reads the parent's file again and takes only its own key. A writer Switchboard does not run — a bare CLI invocation, a third-party automation — takes no such lock and can still slip through, yielding that stub rather than corruption. Not wired for the other harnesses in v1 (per resolved 10.14; per-harness ground truth in [harness-behavior.md](harness-behavior.md) §3.5).
- **Compact context** — ask the harness to summarize the conversation so far and continue from the summary, the headless equivalent of the interactive `/compact`. **Claude only** (`supports_manual_compaction`; the matrix in §9 and [harness-behavior.md](harness-behavior.md) §3.9 record why the other harnesses answer a `/compact` *prompt* with a fabricated success rather than a refusal). It behaves like a send: an idle agent starts it at once, a busy one queues it in FIFO order behind the in-flight turn and any earlier queued sends, and Stop / cancel-send cancel it — no confirmation dialog, no disabled-while-busy state. The live transcript gets a small row carrying the before/after token counts, which is also the only source the sidebar's context bar has for that turn; after a restart only the harness's own recap marker remains. **A compaction is invisible to forwarding** — a forward from a compacting agent waits for it and then resolves exactly as if the agent were idle, reading the latest real answer from disk, because a compaction changes how the conversation is *stored*, never what the agent last *said*.
- **Context breakdown** — ask the harness what is occupying the agent's context window, and show it per category and per item: system prompt, tools, MCP tools grouped by server, custom agents, memory files, skills, messages, free space. **Claude only** (`supports_context_report`), and the other two are excluded for different reasons: **Codex** answers a `/context` prompt from the model with an invented breakdown whose every figure reads like a measurement (probed — [harness-behavior.md](harness-behavior.md) §3.9), while **Antigravity** has no headless `/context` at all (its print mode refuses it — §5.1). It costs nothing — the CLI answers it locally with no model call — but it still writes to the session file, so it queues behind in-flight work like any other turn rather than running beside it. It is **not conversation**: no transcript row at any phase, which is why its outcome lives on the panel instead. The measurement is durable in the harness's own file and re-read on reopen, always labelled with when it was taken, since every turn after it makes it more wrong.
...
- **Source.** What is being sent — any combination of: free-form text the user types, and/or the output from one or more agents (latest turn by default; the user can pick a specific earlier turn). When multiple sources are combined, the optional wrapping prompt is the natural way to control how they're stitched together via template variables. When a source agent is *mid-turn* at send time, the send waits for that turn to complete and forwards its finished output — a cross-agent dependency handled by the dependency-resolution layer described under "Agent contention" (shared with workflows). The whole forwarding capability — both waiting on an in-flight source and forwarding an already-completed one — lands in M6; no forward capability existed before it.
...
- **UI compose-bar dispatch:** a send to a busy agent is **queued**, not refused. Switchboard keeps a per-agent FIFO queue; when the agent's in-flight turn reaches a terminal state, the next queued message dispatches automatically. The queued item owns a transient snapshot of the current model and effort at submission; switching either axis afterward affects later sends, not work already in line. The queued message is visible inline ("queued — agent X is busy") and the user can remove it before it dispatches (its text returns to the compose bar per "Composing and dispatching messages"). The queue is **per-agent and in-memory** — queued-but-undispatched messages and their selection snapshots do not survive an app restart, consistent with in-flight turns (which also don't).
...
**Forward sources may be cross-project.** The ↪ Forward picker offers the current project's agents and panes, plus a `Projects` section listing every other project in an available directory. A source from another project holds, resolves, and composes exactly like a local one; the *recipient* project always owns the send, so the held forward and the resulting turn render there. Each source carries its owning project on the wire (agent id alone names no project to open), which is what lets a source resolve when its project was never opened this session — a chip restored after a restart, for example. Two consequences worth stating: a source's failed/cancelled latest-turn note is read from **its own** project's journal, not the recipient's; and browsing another project's roster in the picker is a **read**, never a load — a project is opened (and locked) only when the user actually picks from it.
...
**Sequencing.** v1 builds these in layers: the per-agent FIFO contention queue lands first (M4.4 — manual sends to a busy agent wait for *that* agent), and is the necessary substrate regardless of what sits above it. The dependency-resolution layer lands with the workflow engine milestone (M6), and is exposed to manual compose-and-dispatch, not only to authored workflows (see v1 plan M6). The compose bar's "forward another agent's output" source is itself an M6 capability — there was no forward source before it. M6 delivers the whole feature at once: forwarding an already-completed turn (the snapshot case) *and* holding the send to wait on an in-flight source turn both arrive together, since both ride the same resolver.
...
- Within a fan-in step (Primitive 4) — or any parallel `send` — a participating agent failing marks the step failed, but Switchboard **never cancels the surviving siblings** (resolved; this supersedes the earlier "SIGTERM the survivors" rule). The policy is phased: **M6** is the non-destructive floor — survivors run to their natural terminal state, their output is retained, the step is `failed`, and retry re-runs the whole step (no partial-state-reuse semantics). **M7** layers an interactive failure-pause on top once the pause machinery (Primitive 5) exists: a sibling failure with ≥1 surviving sibling enters a workflow pause presenting the failed agent + error and each sibling's status/output, with options to retry the failed agent and continue, continue with the survivors' output only (the fan-in helpers then aggregate only the agents that succeeded), or cancel the workflow. If every agent in the step fails (no survivor) it falls back to an ordinary step failure. See `docs/workflow-spec.md` §"Sibling-failure policy" for the full rule and boundary cases.
...
These pin how the §3 split renders concretely after an app restart, when live UI state is gone and the unified transcript is rebuilt by merging the conversation journal (user sends grouped by `send_id`; outcome markers for non-completed turns) with the harness session files (agent-produced content), ordered by timestamp. The three rendered kinds are disjoint — **user messages** come from the journal, **agent content** from harness files, **failed/cancelled markers** from the journal — so the merge never *de-dups* sources against each other. It *does* **correlate** agent content to sends: each content-bearing turn is joined to its send by a durable `hydration_key` (`TurnLink`) where one exists, positional order otherwise (see "Sends and turns" and §3). **One refinement for imported history (example 6):** for an *attached* session's turns that predate journaling — older than the agent's first journaled send — the harness file is the *only* record of the user's prompts, so those user messages are read from the harness file too. The merge stays de-dup-free because it partitions cleanly at that boundary: a turn is either journaled (user side from the journal, harness user-role copy dropped) or pre-journaling (user side from the harness file), never both. Agents below: **B** = Claude, **C** = Codex.

1. **Single completed turn.** You send "hello" to B; it completes. Journal: one `Send`, no outcome (completed turns get none). Harness file: a user-role "hello" and an assistant-role reply. **Renders:** `User → B: "hello"` (from the journal) · `B: <reply>` (harness, assistant-role only). The harness file's user-role copy is dropped — for a turn Switchboard dispatched, the journal is the canonical record of the user's words (it carries the `send_id` grouping the harness file can't). (Caveat: this drop is correct only when the journal *has* the send; see example 6 for imported history where it doesn't.)
...
5. **Cancelled mid-stream.** You send "write a long essay" to B; it streams "Once upon a time…", you hit stop. *Live:* the partial text is visible from Switchboard's in-memory buffer. *After restart:* the buffer is gone, and what's shown is whatever the **harness** persisted plus the journal's `cancelled` marker. Claude Code and Codex persist nothing for an aborted turn, so it renders `User → B: "write a long essay"` · `⚠ B: cancelled` — **marker only, no partial**. A harness that *does* persist partial content (Antigravity — unverified) would render that content above its marker automatically, with no special handling. Switchboard never persists agent content of its own; the post-restart partial is purely a function of what each harness keeps.

6. **Attached session with pre-journaling history.** You attach an existing Claude session that already has two turns — `User: "fix the build"` · `B: <reply>` — done in the `claude` CLI before Switchboard ever saw it. Then you dispatch `"now add tests"` through Switchboard; it completes. Journal: one `Send` ("now add tests"), nothing for the imported turn (it predates journaling). Harness file: both user-role prompts + both assistant replies. **Renders:** `User → B: "fix the build"` · `B: <reply>` · `User → B: "now add tests"` · `B: <reply>`. The imported prompt comes from the **harness file** (the journal never saw it); the dispatched prompt comes from the **journal** (and its harness user-role copy is dropped, per example 1). The boundary is the agent's first journaled send: turns before it take their user side from the harness file, turns at/after it from the journal — so nothing is dropped and nothing is doubled. *(Without this, the imported prompt would survive at attach time — the per-agent transcript load keeps harness user turns — but vanish on restart, leaving an orphaned reply. The merge reads the leading, un-journaled turns' prompts from the harness file. Correlation is a **durable key-join first, positional fallback**: a content-bearing turn with a `TurnLink` is matched to its send by `hydration_key` (and its harness-echoed prompt suppressed via the same reply/provenance pairing), so a stray record can't shift a different send's grouping; only the residual — keyless/legacy/in-flight turns — is decided positionally, which for Claude means the session file's provenance marker (`promptSource`, immune to interleaved `/compact` housekeeping and bare-CLI prompts) with order-alignment as the final fallback when no marker is present.)*
...
Switchboard interacts with Claude Code and Codex through their non-interactive modes (`claude -p`, `codex exec`). The underlying sessions are real harness sessions backed by each harness's own session files — they survive Switchboard, can be resumed later, and could in principle be opened in the harness's interactive TUI by the user if they wanted. Switchboard does not lock the user out of the harness; it just drives it.

The architectural backbone of this section is the **per-harness adapter** pattern (design-pattern sense): one adapter per harness translates that harness's native event stream into a normalized internal stream the rest of Switchboard consumes. This keeps the workflow engine, UI, and persistence layer harness-agnostic, while letting each adapter handle its own quirks (event vocabularies, exit-code semantics, session-file richness, etc.). See "Per-harness adapter and normalized event stream" below for the shape. Per-harness ground truth lives in [harness-behavior.md](harness-behavior.md) (raw probes archived under `research/archive/`).

### Process model

Per-message process spawn for v1: each turn invokes the harness's non-interactive mode — passing the prior turn's session id via that harness's own resume/continuation flag — captures the output, and exits. State persists in the harness's session files between invocations, so a resumed turn restores prior context without Switchboard replaying it. (The exact per-harness flags and argument shapes are an adapter implementation detail — they differ per CLI and shift across CLI versions — so they live in the harness adapters and [harness-behavior.md](harness-behavior.md), not here.) Long-lived agent processes can be considered later if latency matters.

**Antigravity is the structural outlier.** `agy` is a thin client to a server-side agent with a contract unlike the other two: a **control-only** structured stream (`--output-format stream-json`, added agy 1.1.8) — it carries the terminal verdict and tool-failure signals but no thinking text, and stdout's plain-markdown drip can't be used for content because it replays prior answers on resume, so the adapter tails `transcript.jsonl` for all content; the conversation UUID is **server-assigned**, captured from the control stream's `init` event (**primary** since agy 1.1.8), falling back to the per-dispatch CLI log and then to watching for a new `~/.gemini/antigravity-cli/brain/<uuid>/` directory — the fallbacks are retained because only the last of them is prompt-correlated, which is what keeps concurrent same-cwd dispatches from binding each other's conversation. Persisted to the agent's registry record (`AgentRecord.session_locator`) via a normalized capture event the dispatcher writes — first capture, or a fork-and-heal when the resumed conversation expired server-side; resume passes the captured UUID via `--conversation`; and the process exit — not a stream/transcript record — remains the authoritative turn terminator. Exit behavior varies by failure shape in the 1.1.x line (a timeout or empty prompt exits 1; a prompt that is exactly a flag token exits 2), but outcome classification stays **signal-based** and never gates on exit status: the full map is unprobed, and the stream plus stderr signals are sufficient. Auth is macOS-Keychain-only, and a stale token triggers an interactive-OAuth fallback the adapter detects on **stderr** (where agy 1.1.x moved its diagnostics) and force-kills. Full ground truth in [harness-behavior.md](harness-behavior.md).

**Why Antigravity roots under `~/.gemini/`.** `agy` shares Google's config root even though it is a distinct product. Switchboard also shipped a **Gemini CLI adapter until 2026-08-27**, when it was removed: Google withdrew Gemini CLI access for individual accounts (Antigravity is its replacement for those tiers), which made the adapter impossible to exercise in live tests or by hand. See `harness-behavior.md` G26 and the removal milestone in `docs/implementation_plans/2026-08-27-cross-project-operations-and-central-store.md`.
...
The capabilities and behaviors Switchboard needs from each harness, with notes on what is exposed natively, derived, or unavailable. Hands-on probe results are distilled in [harness-behavior.md](harness-behavior.md) (raw captures under `research/archive/`).

| Capability | Claude Code | Codex | Antigravity |
|---|---|---|---|
| Spawn with explicit flags | native | native | native |
| Send + capture structured stream | native | native | **control-channel only** (`--output-format stream-json`, added agy 1.1.8): its `result` event is the terminal verdict and its `state:"ERROR"` tool steps are the sole witness to a rejected tool call, but it carries **no thinking text**, so all displayed content is still tailed from `transcript.jsonl` |
| Detect turn completion | native | native | derived (process exit remains the authoritative terminator; the stream's `result` supplies the verdict) |
| Detect errors | native | native (asymmetric payload) | derived (stream `result.error` + the stderr `Error:` line; per-dispatch CLI-log scan as fallback) |
| Resume by UUID | native | native | native (`--conversation <uuid>`) |
| Select model per agent | native (`--model`) | native (`-m`) | native (`--model <slug>`, per invocation; mutates no harness-owned config) |
| Select reasoning effort per agent | native (`--effort`) | native (`-c model_reasoning_effort=`) | native (`--effort`, **paired with `--model`**; valid levels are per-model, and mandatory for models that have the axis) |
| Assign session ID at spawn | native (`--session-id`) | unavailable (captured from stream → registry record) | unavailable (server-assigned; captured from `brain/<uuid>/` → registry record) |
| Fork from checkpoint | native (`--resume` + `--session-id` + `--fork-session`) | headless via `codex app-server` `thread/fork` only (2026-08-10 probe); not wired in v1 (per resolved 10.14) | unavailable |
| Read context window in stream | native (`result.modelUsage`) | unavailable (session-file only) | no context window anywhere (server-side); token counts *are* on the stream's `result.usage` but are deliberately unconsumed — with no window they cannot drive the context bar |
| Read cost in stream | native (`total_cost_usd`) | unavailable (subscription quota model) | unavailable (server-side) |
| Read rate-limit / quota | native (`rate_limit_event.unifiedWindows` on the stream — **undocumented**, found by probe, guarded by a live test) | native (`account/rateLimits/read` over `codex app-server` — an on-demand account query, **undocumented**, guarded by a live test) | available but **deliberately unbuilt** (`agy -p "/usage"` returns both windows for both model groups; costs seconds per turn — see harness-behavior.md §5.1) |
| Tool calls + results in stream | native (typed blocks) | native (`command_execution` only) | derived (calls and results from the `transcript.jsonl` tail; tool **failures** only from the control stream — the one shape disk never records) |
| Manual compaction | native (`-p "/compact"` on a resumed session; the CLI intercepts it locally) | headless via `codex app-server` `thread/compact/start` only (2026-09-16 probe); not wired — `codex exec` cannot compact at all | unavailable (built upstream, behind the remote feature flag `enable-compact-slash-command`) |
| Context breakdown | native (`-p "/context"` on a resumed session; intercepted locally, answers with a structured object, costs nothing) | unavailable — answered by the *model* from inference, which is worse than absent | unavailable (print mode refuses `/context`; exit 2) |
| Capture permission denials | native (`result.permission_denials`) | presumed (verification deferred) | n/a (auto-approved via `--dangerously-skip-permissions`) |
| Run agents concurrently | confirmed | presumed | confirmed (distinct server-assigned UUIDs; no prefix-collision hazard) |
| Durable send↔turn correlation | native for ordinary turns (key-join on first assistant `message.id`); imported/local-command compatibility output is positional | native (key-join on `turn_context.turn_id` via enrichment; probe-verified) | **positional only** (no per-turn id — order/count fallback; G24) |

**Per-row notes** (only where the matrix needs elaboration):

- **Send + capture structured stream**: Claude and Codex each emit a structured JSON event stream in their non-interactive modes (the adapter requests it per CLI — illustratively `--output-format stream-json` / `--json`). **Antigravity's structured stream is control-only** — `agy` gained `--output-format stream-json` at 1.1.8 and the adapter requests it on every dispatch, but it carries no thinking text, so it supplies the terminal verdict and the tool-failure signal while **all displayed content** still comes from the `transcript.jsonl` tail (stdout's markdown drip is unusable — it replays prior answers on resume); see "Antigravity is the first adapter that builds its live stream from a file tail" above.
- **Detect turn completion**: single terminal event per turn for the two structured-stream harnesses. Claude: `result`. Codex: `turn.completed` (success) or `turn.failed` (error). The adapter waits for whichever terminal shape the harness emits. **Antigravity is the exception** — its control stream's `result` event supplies the terminal *verdict*, but **process exit** remains the turn *terminator* (see Process model). The two are deliberately distinct: the verdict says how the turn went, the exit says when it is over.
- **Detect errors**: Claude uses `result.is_error` and/or `result.api_error_status` (do **not** rely on `result.subtype` — stays `"success"` even on error). Codex: `turn.failed` event terminates the turn with the API error in the payload. Both exit non-zero on error. **Antigravity is the exception** — its verdict comes from the control stream's `result` event, backed by the stderr `Error:` / `Authentication required` lines (agy 1.1.x moved diagnostics off stdout) and a per-dispatch CLI-log scan as fallback. Exit behavior varies by failure shape rather than being uniformly 0, but the adapter deliberately never gates outcome on exit status — the full map is unprobed and the signals above are sufficient.
- **Assign session ID at spawn**: Claude lets the caller pass `--session-id <uuid>`. Codex and Antigravity assign their own server-side and Switchboard captures it (Codex from the first stream event; Antigravity from its stream `init` event, with the CLI log and `brain/<uuid>/` directory as fallbacks) — both persist the captured locator to the agent's registry record (`session_locator`) via a normalized capture event the dispatcher writes.
- **Select model / reasoning effort per agent**: model and effort are two independent per-agent axes with different per-harness capability sets (`HarnessKind::supports_model_selection` / `supports_effort_selection`; full ground truth in [harness-behavior.md](harness-behavior.md) §3.3/§3.4). Each axis has an ordered quick-choice set and one current value; Settings owns the equivalent per-harness sets and independent defaults used by Add Agent and new-project auto-seeding. Creation resolves those defaults into a dispatch-ready current pair without changing either choice set. The shared editor uses multi-select toggles plus an explicit default/current selector only when needed. In the sidebar, one choice is metadata, two choices switch in one click, and three or more use a menu. Changing one axis preserves the other whenever the resulting pair is valid, and the complete selection is persisted atomically. The current pair is captured per send and both values are sent on every turn when set (idempotent for Claude, required per-invocation for Codex/Antigravity). **Both axes are drivable on all three harnesses.** (Antigravity was the holdout until agy 1.1.19 made both per-invocation flags; the former lever, its global settings file, is harness-owned and remains untouched.) Antigravity is also the only harness where effort is **model-dependent and sometimes mandatory** — `HarnessKind::effort_requires_model` — so incompatible quick switches are disabled and model changes resolve to a compatible configured effort. Per-turn model is recorded on the transcript for all model-reporting harnesses (Antigravity via carry-forward of its announced model); per-turn effort is recorded for Codex, Claude, and Antigravity.
- **Fork from checkpoint**: Claude `--fork-session` with `--resume`, plus `--session-id` to choose the forked session's id — so Switchboard pre-generates the fork's locator at registration, same as a fresh Claude agent (2026-08-10 re-probe; previously assumed to require capturing the id from `system/init`). Codex: `codex exec` has no fork, but the experimental `codex app-server` exposes a headless `thread/fork` (found 2026-08-10) — unwired in v1 per resolved 10.14. Antigravity has no equivalent. The Fork affordance — the send button's second half — is offered only on Claude Code agents and is hidden for the others; the `⇧⌘↵` shortcut stays live and refuses with the reason, so the explanation survives the control being hidden. Full mechanics in [harness-behavior.md](harness-behavior.md) §3.5.
- **Read context window in stream**: Claude is the only harness with this stream-side. Codex enriches from the session file's `task_started.model_context_window` per resolved 10.15. Antigravity exposes no **context window** anywhere (server-side), so its bar is hidden too — though it does report raw token counts on the control stream's `result.usage`, which the adapter deliberately leaves unconsumed: without a window they cannot drive the bar (per the per-harness sidebar matrix in §7).
- **Read cost in stream**: Claude only (Agent SDK credit pool post-2026-06-15). Codex doesn't expose dollar costs under subscription auth; Antigravity surfaces no cost data at all.
- **Read rate-limit / quota**: Claude and Codex, in different shapes. **Codex is asked rather than reported to**: `account/rateLimits/read` over `codex app-server` returns every metered quota keyed by `limit_id`, outside any turn, costing no quota and answering while the account is capped (class E). It replaced a session-file read that could only ever carry one quota of N without saying which. Claude emits one on the stream carrying a `unifiedWindows` map — every key becomes its own meter — plus a second event when a window passes its warning threshold; that payload is stream-only, so it is persisted to the metadata sidecar and shown with a snapshot age after a restart. **Neither field is documented by its vendor**, and Claude's was found only by probing, so `live_claude_rate_limit_carries_unified_windows` exists to notice if it moves. Antigravity surfaces no comparable signal.
- **Context breakdown**: Claude only. The CLI intercepts `/context` locally and answers with both a structured `context_usage` object and the printed table, at no model cost, which is what makes the panel a measurement rather than a scrape. On Codex the same command is answered by the model from inference — a fabricated breakdown whose every number reads like a fact — so the capability predicate refuses rather than asks. Antigravity is excluded for the plainer reason that its print mode has no `/context` to call (§5.1); what its *model* would do with the string is untested, because Switchboard passes `--disable-slash-commands` on every Antigravity dispatch. Full protocol, the disk-parser routing, and why terminal-run reports stay uncaptured are in [harness-behavior.md](harness-behavior.md) §3.11.
- **Tool calls + results in stream**: structurally divergent across harnesses. Claude emits typed `tool_use` / `tool_result` content blocks with named tools (including MCP). Codex routes everything through `command_execution` items (raw shell commands with `aggregated_output` and `exit_code`). Antigravity recovers tool calls and results from the `transcript.jsonl` tail (`PLANNER_RESPONSE.tool_calls` → `ToolStarted`; the matching tool-result record → `ToolCompleted`), paired by **exact expected step** — `result_step = planner_step + 1 + call_index`, never arrival order. Its control stream carries one tool event and only one: a `state:"ERROR"` step for a call agy rejected, which is the sole witness to a failure it writes no transcript record for (G32). Switchboard renders these differently per harness; no single unified rendering. **No raw token counts are surfaced in the UI for any harness** — tokens are plumbed through the event stream and used internally (context utilization, debugging), but the user-facing surface is dollars (Claude) or quota (Codex) or empty (Antigravity).
- **Permission denials**: Claude's `result.permission_denials` is informational (not a turn-error); the model receives them as feedback and adapts. Codex behavior is presumed similar but unverified — §7 wording is hedged accordingly. Antigravity runs with `--dangerously-skip-permissions`, so tool calls are auto-approved and no denial signal is produced.
- **Run agents concurrently**: Claude confirmed via three parallel `claude -p` invocations producing three independent session files. Codex's process model and per-session-file isolation suggest the property holds; explicit verification deferred. Antigravity gets a distinct server-assigned UUID per conversation (no filename-prefix collision hazard), and the adapter correlates each dispatch to its conversation by exact prompt match — so byte-identical concurrent prompts in one cwd fail loud rather than mis-bind.

### Per-harness adapter and normalized event stream

The three harness streams (Claude Code, Codex, Antigravity) are structurally different (event-name vocabularies, content shapes, where context-window / rate-limit / cost info appears, what's in the session file vs. stream-only). To keep the rest of Switchboard harness-agnostic, the harness layer is organized around **per-harness adapters** that translate native events into a normalized internal event stream the rest of the system consumes.
...
| `RateLimitEvent { agent, info }` | Harness-specific quota signal, surfaced for UI display. The workflow engine doesn't interpret it. **Claude** emits it on the live stream with no session-file equivalent (class C) — so it's persisted to the per-agent metadata sidecar (`<agent-id>.meta.json`) to survive restart. **Codex no longer emits it at all** — its quota comes from the account query (class E), which is not a turn event and reaches the frontend through its own command. An internal `RateLimitSource` discriminator (`StreamOnly` vs `SessionFileBacked`) decides persistence and is **dropped at the wire boundary** — the frontend doesn't see it; `SessionFileBacked` has no production producer since Codex's quota moved. Antigravity emits no rate-limit signal. |
...
- **`Failed { kind: AuthFailure, message }`** — auth is missing or expired. Detected per-harness: Claude `assistant.error == "authentication_failed"`; Codex `turn.failed.error.message` contains `"401 Unauthorized"`; Antigravity's `Authentication required` / `authentication timed out` lines, matched on **stderr** (where agy 1.1.x moved its diagnostics) with the stdout scan retained as a backstop. The exact 1.1.x auth shape is **unverified** — probing it requires logging the developer out — so both streams stay wired. Each adapter **authors** an actionable message (names the harness + recovery command, e.g. `claude auth login` / `codex login`); the transcript renders it like any other failed turn. There is **no auth banner** — auth is reactive-only (§37). The `kind` is retained on the wire for future retry-policy decisions, not a per-kind render.
...
| Field | Claude (live) | Claude (rehydration) | Codex (live + rehydration) | Antigravity (live + rehydration) |
|---|---|---|---|---|
| `model` | `system/init` stream event | session-file `system/init` record | session-file first `turn_context` record | parsed from the `USER_INPUT` record's `<USER_SETTINGS_CHANGE>` envelope (only present when the model changed — empty otherwise; the reducer keeps the prior model on empty) |
| `harness_version` | `system/init.claude_code_version` | session-file `system/init` | session-file `session_meta.cli_version` | live: lazy `agy --version` (cached `OnceLock`); rehydration: empty |
| `mcp_servers` | `system/init.mcp_servers` (Claude pre-merges scopes) | Switchboard reads `~/.claude.json` (user + local) + `<cwd>/.mcp.json` (project) | Switchboard reads `~/.codex/config.toml` + `<cwd>/.codex/config.toml` | Switchboard reads `~/.gemini/config/mcp_config.json` (user scope only) |
| `skills` | `system/init.skills` (Claude pre-merges scopes) | Switchboard directory-scans `~/.claude/skills/` + `<cwd>/.claude/skills/` | Switchboard directory-scans `~/.agents/skills/` + `<cwd>/.agents/skills/` | Switchboard scans `~/.gemini/config/plugins/<plugin>/skills/<skill>/SKILL.md`, displayed as `<plugin>/<skill>` (user scope only) |
| `tools` | `system/init.tools` (merged builtin + MCP + dynamic list) | empty | empty | empty |
...
- **`~/.agents/skills/` is a shared multi-harness user-scope path** used by Codex. Retained as a shared path in case another harness adopts it.
...
The `tools` field is preserved across the wire for the populated Claude-live path; it's reserved for a future symmetric registry surface across harnesses (Codex has no `init`-event tools list to draw from).

#### Notes

- **Codex session-file dependency.** Reading Codex session files in addition to the `--json` stream is a committed v1 dependency for the Codex adapter (per resolved 10.15) — needed to fill in gaps the stream doesn't expose (`model_context_window`, full reasoning blocks, `session_meta`). Multiple §7 and §9 commitments depend on this enrichment.
...
### Claude slash-leading messages

For a Claude agent, the ordinary composer has one invariant: a send is model input, not an implicit harness command. Typing `/` into an empty composer opens Switchboard's prompt picker; an unmatched query can be inserted back into the message. At dispatch, Claude receives a transport-only leading ASCII space for any prompt whose first character is `/`, bypassing Claude Code's local command parser while the journal and transcript retain the user's exact text. Other harnesses receive the text unchanged and retain their native headless semantics.

Claude-local commands (`/model`, `/clear`, `/plugin`, user-invoked skills, custom commands, and MCP prompts) are not invoked through ordinary sends. If Switchboard adds them later, they require an explicit command action whose semantics and headless availability are visible before execution. Auto-invoked skills remain unaffected (see §6).

### What we lose by going non-interactive

The interactive Claude Code, Codex, and Antigravity TUIs are not used. Switchboard renders the output itself. This means rendering tool calls, diffs, todo lists, and thinking blocks is Switchboard's responsibility.

What is **preserved** because the harness still runs in default mode: hooks fire, MCP servers connect and tools work, auto-invoked skills trigger normally, sub-agents (Claude Code's `Agent` tool, Antigravity's `invoke_subagent`) spawn as expected, auto-compaction runs when context climbs. Cross-harness, a sub-agent invocation surfaces in Switchboard as **a single tool call from the parent's view** (one `ToolStarted` + one `ToolCompleted` for the delegation tool itself); the sub-agent's internal tool calls are not separately rendered. See [`docs/harness-behavior.md`](harness-behavior.md) §6 for the operational evidence.

What is **lost** in headless mode:

- **Plan mode** (Claude Code's interactive plan/approve cycle) — REPL-only; no headless equivalent.
- **Implicit user-invoked Claude commands** — ordinary Claude sends are always model input. Interactive commands such as `/model`, `/clear`, and `/plugin` require a future explicit command surface if Switchboard chooses to expose them.
- **Manual compaction — on Codex and Antigravity only; it was never actually lost on Claude.** `/compact` is a slash command the Claude CLI marks `supportsNonInteractive`: passed as the positional it is intercepted locally, the model never sees it, and Switchboard ships it as the **Compact context** action (resolved 10.11). Codex's headless equivalent lives only on the experimental app-server and Antigravity's is feature-flag-gated, so for those two it is lost — and worse than lost, because a `/compact` **prompt** is answered by the model *claiming* the context was compacted while nothing is. That is why the capability is a per-harness predicate (`supports_manual_compaction`) rather than a prompt sent everywhere; [harness-behavior.md](harness-behavior.md) §3.9 carries the mechanism, the event protocol, and the residuals. All three auto-compact on their own regardless.
- **The harness's own TUI rendering** — Switchboard renders everything itself from the stream.

### Integration testing

Switchboard's per-harness adapters are exercised by a **live-harness test suite** that runs against the real, installed Claude Code, Codex, and Antigravity CLIs — not mocks. This is critical: adapter correctness depends on harness behavior we don't control (event vocabularies, exit codes, stream timing, session-file format), and fixture-only tests would silently lock in our current understanding while upstream releases drift. Live tests catch those regressions when a developer runs them locally.
...
To keep the live suite affordable in time and subscription cost, every test prompt is constrained to a small response (e.g., "reply with the single word 'ack'", not "write me a poem"). Modern Claude / Codex usage limits are generous enough that even a thorough suite runs in minutes for a negligible cost per run. The constraint is per-test response size, not test count.
...
- **Granular permission / sandbox config.** v1 runs all three harnesses with maximum autonomy (skip-permissions on); the off / restricted-mode user experience is deferred. Plausible future directions: **config-driven tool allowlists** (YAML per project / per agent, passed through as `--allowedTools` / `--permission-mode`), **interactive permission prompts** (Switchboard intercepts denials at runtime and pops a modal asking the user to allow / deny / always-allow, then re-issues the turn — pending a probe of harness resume-after-denial mechanics), and **per-workflow permission scoping** (a workflow step declares its required tools, Switchboard restricts the harness for the duration). Codex's separate approval-policy vs sandbox-mode distinction is part of this same design space — collapsed into the single max-autonomy posture in v1.
...
- ~~**5.2** Passthrough mechanism for Claude commands — namespacing.~~ **Resolved for Claude:** the ordinary composer never invokes Claude-local commands. Switchboard prompts occupy the `/` picker; future Claude commands require an explicit action. Other adapters retain their documented native CLI behavior.
...
- **10.10 (monitoring)** Headless Claude slash-command support. Claude Code now executes some commands in `-p` mode: user-defined commands, skills, and MCP prompts may expand into a model turn, while built-ins such as `/plugin` can return zero-turn local feedback. Switchboard deliberately bypasses that parser for ordinary Claude sends. A future explicit command surface should reevaluate support command-by-command; the Agent SDK remains an alternative transport. Historical upstream context: [anthropics/claude-code#837](https://github.com/anthropics/claude-code/issues/837) and [#38505](https://github.com/anthropics/claude-code/issues/38505).
- ~~**10.11** Compaction strategy.~~ **Resolved: manual compaction is a per-harness action, shipped for Claude; Switchboard still writes no summaries of its own.** The premise that headless `/compact` did not exist was wrong — it is a slash command the Claude CLI marks `supportsNonInteractive`, discoverable only by reading the binary's command table, not `--help` (2026-09-16 probe; [harness-behavior.md](harness-behavior.md) §3.9). Claude agents therefore get a **Compact context** action that queues like a send; Codex's equivalent needs the experimental app-server (the same transport cost as resolved 10.14) and Antigravity's is behind a remote feature flag, so both stay unwired and the action is hidden for them. The two standing parts of the old assumption hold: Switchboard does **not** implement its own summarization, and the auto-compact threshold (`--autocompact` / `autoCompactWindow`) remains the harness's knob rather than a Switchboard setting — a user who sets it in `~/.claude/settings.json` already gets that behavior on every dispatch. The "fork from checkpoint with summary" alternative is dropped; real compaction supersedes it.
- ~~**10.12** Model→max-context map maintenance.~~ **Resolved (commitment).** No bundled model→max-context map ships in v1. Each harness sources the value from whatever it makes available: Claude reads `contextWindow` per turn from the stream's `result.modelUsage.<model>`; Codex enriches `task_started.model_context_window` from the session file post-terminal (per resolved 10.15); Antigravity exposes no context window at all, so `usage.context_window` stays `None` and the per-agent context-utilization bar is hidden for its agents (per the §7 sidebar matrix). The harness-asymmetric sourcing is documented in the §9 SessionMeta source table; the "what the user sees" implications are documented in the §7 sidebar matrix.
- **10.13 (monitoring)** Compaction as a **workflow step**, and a Codex path that does not need the app-server. The manual half of this closed with 10.11 — Claude's headless `/compact` existed all along and now backs the Compact context action — so what remains open is compaction the *workflow* can schedule rather than the user, and the Codex/Antigravity asymmetry. The open Anthropic feature requests ([anthropics/claude-code#5643](https://github.com/anthropics/claude-code/issues/5643), [#39275](https://github.com/anthropics/claude-code/issues/39275), [#39574](https://github.com/anthropics/claude-code/issues/39574), [#26488](https://github.com/anthropics/claude-code/issues/26488)) track an *officially documented* surface, which is worth having given the current one is an undocumented command-table property; Codex's equivalent remains undocumented outside the app-server RPC.
- ~~**10.14** Codex non-interactive fork.~~ **Resolved for v1: option (a).** Fork is unavailable for Codex agents in v1 — the Fork affordance is offered only on Claude Code agents and is hidden for Codex agents; pressing `⇧⌘↵` there refuses with the reason rather than silently doing nothing. Workarounds (b) copy session JSONL and (c) re-feed summarized prior context are deferred to v2+ if user demand surfaces. The asymmetry is documented in §9 and the M4 deliverables. **2026-08-10 update:** a headless Codex fork path now exists — the experimental `codex app-server` exposes a `thread/fork` JSON-RPC method (probed @ 0.146.0; see [harness-behavior.md](harness-behavior.md) §3.5). The v1 decision stands, but the deferral rationale changes: no longer "no headless path," now "requires driving the app-server as a second Codex integration surface alongside `codex exec` — a new process dependency deferred pending user demand."
- ~~**10.15** Should the Codex adapter read the session file (`~/.codex/sessions/...jsonl`) in addition to the `--json` stream?~~ **Resolved (commitment).** The Codex adapter reads the session file on turn completion to enrich the normalized event stream with fields the `--json` stream omits (`model_context_window`, full reasoning blocks, `session_meta` for the SessionMeta event; rate limits were read here too until 2026-09-19, when they moved to an account query). Multiple §7 and §9 commitments now depend on this (per-turn context-utilization for Codex, RateLimitEvent timing, SessionMeta). What remains as implementation detail: tail-vs-completion timing for any future live-stream enrichment, and the lookup-strategy mechanics for the date-partitioned session path (see the archived `research/archive/codex-cli-observed.md` research note).
