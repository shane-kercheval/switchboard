...
- `docs/implementation_plans/2026-05-12-v1.md`'s deferred-detail callout ("Persistence schema details (10.3)") — note that M1 lands the registry + project-index shapes; the runs/checkpoint shape is still M5.
...
1. **Normalized event types.** Two enums: `AdapterEvent` (what adapters emit) and `NormalizedEvent` (what crosses IPC). The split makes "TurnStart is dispatcher-owned, never adapter-emitted" a type-level invariant rather than a doc convention.
   ...
   **Why `kind` on `Failed` (not just `message`):** the discriminator is load-bearing for M5's partial-failure rule (the human-in-the-loop pause we filed in `docs/implementation_plans/2026-05-12-v1.md` M5). When one agent in a fan-in fails, the workflow needs to distinguish "harness reported a model error" (`HarnessError` — bad prompt, retry won't help) from "subprocess crashed" (`AdapterFailure` — transient, retry might help). The UI can use the same discriminator to surface different affordances ("retry" vs "this is a Switchboard problem, restart the agent"). Internal Rust code at the adapter layer can carry richer cause detail in logs without surfacing it on the wire.
...
These surfaced during implementation and live testing. Each needs a decision: support it, document it as a known limitation, or defer to a later milestone.

**Session file deleted manually (or by external tooling).** `~/.claude/projects/<cwd>/<uuid>.jsonl` is Claude Code's session store. If it's deleted while Switchboard still has the agent in its registry, the next dispatch will see "no file → use `--session-id`", Claude Code creates a fresh session, and all prior conversation context is silently lost. Switchboard state (the registry entry) remains intact; the agent keeps working but its history is gone. **Decision needed: warn the user? detect and surface? or document as known limitation?**

**`CLAUDE_CONFIG_DIR` or non-standard Claude storage path.** `ClaudeCodeAdapter` hardcodes `~/.claude` as the session file root. If the user (or a future Claude Code release) changes the config directory via an env var or config option, the file check always returns false, and every dispatch uses `--session-id`, silently creating orphan sessions while the real session accumulates history elsewhere. **Decision needed: read Claude's config dir at startup? or document as unsupported?**

**Concurrent Switchboard instances sharing the same working directory.** Two instances running simultaneously against the same directory could both reach `session_file_exists() = false` for the same agent before either creates the file, then both issue `--session-id` for the same UUID. One succeeds, the other's behavior is undefined (may error, may overwrite). **Decision needed: instance lock (already planned for M3 via `instance.lock`) fully mitigates this — confirm M3 closure or add a note.**

**`--session-id` vs `--resume` is an undocumented internal coupling.** The session file path (`~/.claude/projects/<encoded-cwd>/<uuid>.jsonl`) and the encoding scheme (replace `/` with `-`) are Claude Code internals, not a documented API. A Claude Code update could change the path without notice, silently breaking the first/subsequent-turn distinction. The fallback is benign (start a new session) but silent. **Decision needed: acceptable risk for v1? or add a compatibility check at startup?**
...
4. **Tauri commands.** Working-directory + project lifecycle commands take or return path / id strings; per-agent commands take `agent_id` (no `project_id` needed since agent_ids are globally unique and the dispatcher routes via `AgentRecord.project_id`).
   ...
   **AgentRecord lookup for `send_message`** — deterministic: scan `AppState.projects` for a project whose `registry.jsonl` contains the requested `agent_id`. If found, dispatch with that project's root + the agent's harness type. If the agent is not in any loaded project, return a typed `AgentNotFound { agent_id }` error to the caller. **No implicit "active project" routing** — the agent_id is globally unique (UUID v7), and routing is by lookup, not by UI state. For M1, scanning all loaded projects' registries on each `send_message` is fine (registries are small, M1 has one loaded project). An in-memory `HashMap<AgentId, (ProjectId, AgentRecord)>` cache on `AppState` is a reasonable M3+ optimization if send rate becomes a concern in workflow contexts.
