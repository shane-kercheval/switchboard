...
- **Keychain secret store** — a thin store over the **`keyring` crate** (store/fetch/delete a bearer string by provider key). Introduced in M2 (consumed by `McpProvider`), populated by the generic "Add MCP server" form in M3. (The deferred Tiddly preset's Auth0 device-flow polling + PAT minting — pure logic over an injected HTTP client — is M5; it reuses this same store.)
...
**Implementation Outline.**
- Keychain secret store: introduced in M2 (store/fetch/delete by provider key; `McpProvider` already resolves keychain bearers through it). M3 *populates* it via the Add form below.
...
- **Provider status:** each configured provider surfaces a credential/connection status (`ok` / `token missing` / `unreachable` / `auth failed`) derived from the last cache build, rather than failing silently. An expired or invalid bearer surfaces as "auth failed" and the user re-enters the token — V1 generic providers do **not** auto-renew (that's M5's reconnect machinery).
...
- A `#[ignore]`-gated **Tiddly live test** — this is where the live-test runner-home work lands (per §7): add `switchboard-prompts` to `LIVE_PKGS` + a `make test-live-tiddly` target and document a `live_tiddly_` category in `AGENTS.md`, **or** house it in `switchboard-app`.
