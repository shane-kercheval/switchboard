...
describe("project staleness refresh", () => {
  ...
  it("restores the refusal an agent is still sitting on, and only that", async () => {
    // Verbatim shape of a Codex weekly-cap refusal on reopen: the journal has
    // the failed outcome, and the harness file has the refused turn itself
    // (Codex writes `task_started … task_complete{error}` for it), starting a
    // couple of seconds after the dispatch the journal recorded. That turn is
    // the failure, not something after it; a genuinely later turn is.
    const ws = await loadWorkspaceState();
    const state = await loadAgentState();
    const refusedAgent = AGENT_1;
    const recoveredAgent = AGENT_2;
    installBackend([agent(refusedAgent, PROJECT_1), agent(recoveredAgent, PROJECT_1)]);
    const refusal = {
    ...
    conversation = {
      items: [
        // Refused agent: the journal's outcome at dispatch time, then the
        // harness's own record of that same send two seconds later.
        {
          ...refusal,
          turn_id: "t-r",
          send_id: "s-r",
          agent_id: refusedAgent,
        ...
        },
        {
          ...
          agent_id: refusedAgent,
          send_id: "s-r",
          started_at: "2026-09-18T22:30:02Z",
        ...
        // Recovered agent: refused once, then a different send ran after it.
        {
          ...refusal,
          turn_id: "t-x",
          send_id: "s-x",
          agent_id: recoveredAgent,
          at: "2026-09-18T22:30:00Z",
        },
        {
          kind: "agent_turn",
    ...
    await vi.waitFor(() => expect(state.runtimes[refusedAgent]?.usage_limit_reached).toBe(true));
  ...
  it("reads an unknown journaled failure kind as not-a-usage-limit, leaving the meter alone", async () => {
    ...
    expect(await ws.activateProject(PROJECT_1)).toBe("activated");
    await vi.waitFor(() => expect(state.runtimes[AGENT_1]?.usage_limit_reached).toBe(false));
  });

  it("drops the refusal when the turn after it carries no send correlation", async () => {
    // `agent_turn.send_id` is nullable (a declined anomalous link with no
    // positional match). Identity is then unavailable, the timestamp rule sees
    // the refusal's own turn as a later one, and the refusal is discarded —
    // the safe direction, since the opposite error claims "limit reached" for
    // an agent that has since worked.
    const ws = await loadWorkspaceState();
    const state = await loadAgentState();
    installBackend([agent(AGENT_1, PROJECT_1)]);
    conversation = {
