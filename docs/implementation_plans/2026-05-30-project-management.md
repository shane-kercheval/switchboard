...
- **Schema (additive, `#[serde(default)]`).** Add `archived: bool` to `ProjectConfig`, `ProjectSummary`, and the `ProjectListing` wire type (+ the TS `ProjectListing`). `serde(default)` makes old `config.yaml` / `projects.jsonl` / `workspace.yaml` read as `archived: false` — no version bump, no migration. `workspace.yaml`'s cached `ProjectSummary` carries it automatically.
- **Core (`Directory::set_project_archived(id, archived) -> Result<ProjectSummary>`).** Same dual-write as rename: rewrite `config.yaml` and the `projects.jsonl` entry with the new flag.
- **App (`set_project_archived_impl(state, project_id, archived) -> Result<ProjectListing>`).** Under `registry_write`, **no dispatcher interaction** (display-only). Update `AppState.projects`, `refresh_cache` + `persist_workspace`, return the listing. `list_projects_impl` carries `archived` through both the fresh-read and cached paths.
- **Frontend.** `ProjectActionsMenu` shows Archive or Unarchive based on `project.archived`. `workspace.svelte.ts` `setProjectArchived(projectId, archived)` replaces the listing in `projects.list`. `ProjectsSidebar` header gets a "Show archived" toggle (`$state` local); the rendered list is a `$derived` filter — exclude archived unless the toggle is on; archived rows get a muted treatment + the Unarchive affordance.

### Definition of Done

- Core tests: archive/unarchive round-trips through both files; old records without the field default to `false`.
- App tests: `set_project_archived_impl` flips the flag, persists, returns the listing; a running agent in the project is unaffected (no dispatcher call).
- Component tests: archived rows hidden by default; toggle reveals them de-emphasized; Archive/Unarchive item reflects current state and calls through.
...
- `docs/system-design.md` §3 — note the `archived` field on the project config/summary and that `projects.jsonl` is rewrite-on-mutation (like `registry.jsonl`); confirm the delete blast-radius (Switchboard state only, never harness files).
