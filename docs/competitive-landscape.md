...
### [VS Code Agents Window](https://code.visualstudio.com/docs/copilot/agents) (Preview — May 2026)

A dedicated agent-first window alongside the VS Code editor, shipping in v1.120 (May 13 2026). Open with `code --agents` or the "Open in Agents" button. Four panels: sessions list (all workspaces), customizations panel (Agents, Skills, Instructions, Hooks, MCP Servers, Plugins), chat area, and changes panel (diff view + file explorer). Supports Copilot CLI, Copilot Cloud, and Claude agent sessions. Sub-sessions for parallel tasks in the same workspace. Remote sessions via SSH or dev tunnel.

**Custom Agents** are `.agent.md` files with YAML frontmatter (`tools`, `agents`, `model`, `handoffs`, `hooks`), stored in `.github/agents/` (workspace) or `~/.copilot/agents/` (user). VS Code also reads `.claude/agents/*.md` in Claude sub-agent format — meaning agent definitions are cross-compatible with Claude Code sub-agents. **Handoffs** are the most relevant feature: sequential workflow buttons that appear after a response (e.g., Planning → Implementation → Review), each optionally pre-filling a prompt with `send: true` to auto-submit. This is a lightweight version of Switchboard's `pause_for_user` Mode 2 — human-approved step transitions with optional auto-dispatch — but scoped to a single IDE session with no fan-out, fan-in, or YAML workflow files. **Agent Skills** (agentskills.io open standard) are portable instruction folders that work across Claude Code, Copilot CLI, and cloud agents — a cross-harness prompt/instruction primitive comparable in spirit to Switchboard's prompt library.

**What it does not do:** No fan-out, no fan-in, no cross-agent message routing, no YAML workflow DSL, no harness-agnostic operation (VS Code only). Handoffs are manual button clicks that transition one session sequentially — not automated routing of one agent's output as another agent's input. The user is still the message bus for any actual content transfer between agents.

**What's relevant to Switchboard:** (1) The Handoffs UX pattern — sequential step buttons with optional auto-send — is worth studying as a UI reference for surfacing `pause_for_user` transitions. (2) `.claude/agents/` cross-compatibility means Switchboard could read the same agent definition files VS Code and Claude Code already use, rather than inventing a separate format. (3) The unified customizations panel (agents, skills, instructions, MCP, plugins in one place) is a UX reference for Switchboard's own agent/workflow configuration surface.

**On the handoffs design philosophy — and why Switchboard should not copy it:** VS Code's choice to put handoffs inside the agent definition (rather than in a separate workflow file) reveals a fundamental assumption: the agent is the unit of composition, and the workflow is implicit. A Planner agent that always hands off to an Implementer bakes that relationship into the agent itself. This works for simple linear sequences in a chat-first context — after a response completes, the "next step" button appears right where the user is, which is ergonomically nice.

But the coupling is the problem. The same Planner agent might legitimately hand off to an Implementer in one workflow, an Architect in another, and three parallel reviewers in a third. If the handoff is baked into the agent, you either create separate agent variants per workflow or stuff all possible handoffs into one definition. Neither scales. Worse, when handoffs are distributed across agent files, the workflow is implicit — to understand the full sequence you have to open each file and piece it together.

Switchboard's answer is the inverse: the **workflow is the first-class citizen, and agents are pure capability definitions** (tools, instructions, model, persona). Sequencing, routing, fan-out, fan-in, and pause points all live in the workflow file, not in the agent. An agent file should be reusable across any workflow that needs its capability — it shouldn't know who it's handing off to, or that it's part of any workflow at all. This is cleaner, more reusable, and the only model that can express fan-out and conditional routing without gymnastics.
...
- [VS Code Agents Window docs](https://code.visualstudio.com/docs/copilot/agents) / [Custom agents](https://code.visualstudio.com/docs/copilot/agents#_custom-agents) / [Agent Skills (agentskills.io)](https://agentskills.io/)
