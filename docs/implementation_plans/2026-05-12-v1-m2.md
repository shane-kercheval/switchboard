...
5. **Implementation location.** Display work could land in M2 (extends the existing M1.5 single-pane) or M3 (when multi-agent UI lands). Data plumbing must be M2 regardless.
6. **Relationship to M7 onboarding.** Whatever's shown in-app needs an onboarding explanation of which credit pool / quota gets consumed and the post-2026-06-15 Claude Code split. M7 already has scope for this — keep them coherent.
