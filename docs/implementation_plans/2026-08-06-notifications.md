...
- One case is unverified and belongs in M1's manual check: whether activation also **unminimizes** a minimized window. If it does not, handle it on the app's own activation event — still without observing notification responses. `tauri::RunEvent::Reopen` (confirmed present in 2.11.1, mapped from `applicationShouldHandleReopen`) is the candidate, with three caveats: it is **not** notification-specific, so wiring it also changes Dock-click behavior; restoration must **not** be gated on `has_visible_windows`, because macOS counts a minimized window as visible — check the window's own minimized state; and whether a notification click reaches that delegate at all is itself unverified, so a fix must be re-checked against a **notification click**, not a Dock click, which would pass either way. If `Reopen` turns out not to fire for notification clicks, the fallback is a notification action button (which moves `response()` into the delegate-only branch, avoiding both the poller and the auto-delete) — accepted only as a last resort, since it puts a visible, meaningless button on every notification.
