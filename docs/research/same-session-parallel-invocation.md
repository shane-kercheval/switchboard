...
6. **Attach-time session-id uniqueness scope follows each harness's session-file namespace.** The dispatcher gate above stops *intra-process* concurrent drives; the attach-time uniqueness scan stops two `AgentRecord`s from pointing at one harness session in the first place. Its scope must match how the harness names session files: **Claude and Gemini are cwd-namespaced** (`~/.claude/projects/<encoded-cwd>/…`; Gemini under a cwd-resolved `~/.gemini/tmp/<project-name>/`), so the same session-id under a *different* working directory is a genuinely *different* session — uniqueness is enforced **per working directory**. **Codex and Antigravity are globally addressed** (`~/.codex/sessions/<date>/…`; `~/.gemini/antigravity-cli/brain/<uuid>/`), so the same id is the *same* session regardless of directory — uniqueness must be enforced **across every working directory loaded in the Switchboard workspace** (system-design §3's flat cross-directory model). Widening the Claude/Gemini scan to the whole workspace would false-reject legitimate same-id-different-cwd attaches; leaving the Codex/Antigravity scan at one directory would miss real collisions once the workspace loads multiple directories at once.
