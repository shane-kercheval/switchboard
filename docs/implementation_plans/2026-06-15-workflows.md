...
In the prompt composer (the structured-args form, `PromptComposer.svelte`), each declared argument can be filled by typing **or** by forwarding output from one or more agents/panes — the manual analogue of a workflow `send` whose `template_vars` reference `aggregated_responses(...)`.

- **Per-argument, independent sources.** Each argument gets its own optional "forward from {agent | pane}" affordance; different arguments can draw from different sources. A pane expands to its member agents (frontend-side, membership captured at send time), same as M2.
- **Hold + wait + dispatch.** If any referenced source is mid-turn, the prompt send is **held** (M2's mechanism) until all referenced sources settle; then the argument values are filled with the resolved text, the prompt is rendered via `PromptService::render`, and the rendered message is dispatched to the target recipient(s) (fan-out, like M2). A busy recipient queues (`OnBusy::Enqueue`).
- **Same rules as M2, reused not reinvented.** Empty-source handling, the canonical per-agent block composition for a forwarded argument value, invalidation/cancel-restore, and the after-the-fact partial-empty annotation are identical to M2 and share its resolver + composition function.
- **Empty handling is per-argument.** Within one argument forwarded from several sources, include the non-empty ones; an *optional* prompt argument may resolve empty; a **required** argument that resolves fully empty (all its sources empty) is a validation failure → restore (consistent with the prompt composer's existing required-arg gating).

### Implementation Outline

- Reuse the **M2 resolver** (await sources via the M1 completion path / disk snapshot) and the **M2 canonical-composition function** for each forwarded argument's value. No parallel mechanism.
- Backend command/path: take target recipient(s), the prompt id, the typed args, and per-argument forward sources; resolve each forwarded argument's sources → fill the args map → `PromptService::render(provider, name, args)` → dispatch (held until sources settle). This is the **manual precursor of M4's workflow `send` with `prompt` + `template_vars`** — M4 reuses it. State that linkage in the doc so the two do not drift.
- Frontend: `PromptComposer.svelte` gains a per-argument forward affordance (the same source-chip component as M2's compose bar). Preserve the existing structured-args + preview lifecycle; a forwarded argument shows its source chips in place of (or alongside) the typed value, and the preview reflects the resolved value where known.

### Definition of Done

- Backend tests: resolve sources → fill args → render → dispatch; **mixed** typed + forwarded args on one prompt; held-until-settle; an *optional* arg resolving empty proceeds; a **required** arg whose sources are all empty fails and restores; the forwarded-argument composition is asserted **identical** to M2's shape (shared function).
- Frontend tests (mandatory, per the component-test convention): per-argument forward affordance renders; wait-then-render-then-dispatch; failure/cancel restores text + chips; idle-empty source flagged on its chip.
- Doc states the reuse of the M2 resolver + composition and the explicit "M4 reuses this path" linkage.
