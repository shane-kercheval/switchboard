...
| Detect turn completion | native | native | native | derived (process exit remains the authoritative terminator; the stream's `result` supplies the verdict) |
| Detect errors | native | native (asymmetric payload) | native | derived (stream `result.error` + the stderr `Error:` line; per-dispatch CLI-log scan as fallback) |
| Resume by UUID | native | native | native | native (`--conversation <uuid>`) |
| Select model per agent | native (`--model`) | native (`-m`) | native (`-m`) | native (`--model <slug>`, per invocation; mutates no harness-owned config) |
| Select reasoning effort per agent | native (`--effort`) | native (`-c model_reasoning_effort=`) | **unavailable** (config-only) | native (`--effort`, **paired with `--model`**; valid levels are per-model, and mandatory for models that have the axis) |
| Assign session ID at spawn | native (`--session-id`) | unavailable (captured from stream → registry record) | native (`--session-id`) | unavailable (server-assigned; captured from `brain/<uuid>/` → registry record) |
| Fork from checkpoint | native (`--resume` + `--session-id` + `--fork-session`) | headless via `codex app-server` `thread/fork` only (2026-08-10 probe); not wired in v1 (per resolved 10.14) | lossy `--session-file` import only; not wired in v1 | unavailable |
| Read context window in stream | native (`result.modelUsage`) | unavailable (session-file only) | unavailable (no analog field) | no context window anywhere (server-side); token counts *are* on the stream's `result.usage` but are deliberately unconsumed — with no window they cannot drive the context bar |
...
| Read rate-limit / quota | unavailable (no public stream signal) | native (`token_count.rate_limits` in session file) | unavailable | available but **deliberately unbuilt** (`agy -p "/usage"` returns both windows for both model groups; costs seconds per turn — see harness-behavior.md §5.1) |
| Tool calls + results in stream | native (typed blocks) | native (`command_execution` only) | native (filtered `update_topic`; output empty for read-like tools) | derived (calls and results from the `transcript.jsonl` tail; tool **failures** only from the control stream — the one shape disk never records) |
