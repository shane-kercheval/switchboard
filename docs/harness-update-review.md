...
|---|---|---|---|
| Codex | `0.143.0` | 2026-07-08 | No impact — `exec` flags (incl. `resume`), stream vocabulary, `turn_context`/`token_count` records, and auth/exit-code semantics unchanged. New `world_state` rollout record is additive (parser's unknown-type arm skips it); rollouts stay uncompressed plaintext (latent risk only). |
