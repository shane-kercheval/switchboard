...
- **Deleting is the other direction, and the default gate cannot help you there.** When you remove an adapter event variant or a `LoadedTranscript` field, grep the live tests for the symbol as a step of the deletion. `make check` does not execute those assertions at all — live tests are `#[ignore]`-gated — so one asserting the behavior you just deleted stays green in CI and fails only when a developer next spends quota. This is not the bullet above restated: that one is about adding coverage for something new, this one is about coverage that silently outlives its subject.
