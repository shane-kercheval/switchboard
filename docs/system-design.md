...
└── .switchboard/
    ...
    └── projects/
        └── <project-id>/       # per-project state (one subdirectory per project)
            ...
            ├── sessions/       # per-agent sidecars: Codex session-link
            │                   #   (<agent-id>.jsonl) + stream-only-metadata cache
            │                   #   (<agent-id>.meta.json — restart continuity).
            │                   #   Claude/Gemini/Antigravity session identity
            │                   #   lives on the registry record (session_locator).
...
| Assign session ID at spawn | native (`--session-id`) | unavailable (captured from stream → sidecar) | native (`--session-id`) | unavailable (server-assigned; captured from `brain/<uuid>/` → registry record) |
...
- **Assign session ID at spawn**: Claude and Gemini both let the caller pass `--session-id <uuid>`. Codex and Antigravity assign their own server-side and Switchboard captures it (Codex from the first stream event; Antigravity from the new `brain/<uuid>/` directory). Antigravity persists the captured UUID to the agent's registry record (`session_locator`); Codex still persists to a per-agent session-link sidecar. Note: Gemini uses UUID **v4** (not v7) because Gemini's session-file filename uses only the first 8 hex chars and v7s minted in the same millisecond share that prefix — see `gemini/mod.rs` for the rationale.
