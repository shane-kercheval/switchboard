...
- **Antigravity replies can't be pinned.** Antigravity's transcript does not provide a per-message identity that survives reopening, so Switchboard disables pinning for its replies rather than risk attaching a pin to the wrong message. User messages sent to Antigravity can still be pinned.
