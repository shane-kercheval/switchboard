...
use switchboard_harness::{
    ...
    DispatchOptions, HarnessAdapter, TurnOutcome,
};
use uuid::Uuid;
...
async fn live_antigravity_emits_tool_started_and_tool_completed_for_file_read() {
    ...
    // sentinel-in-output pairing works: the token survives
    ...
    // Non-hidden tempdir prefix, kept deliberately. `agy` used to refuse a
    // *hidden* (dot-prefixed) directory as a workspace ("is hidden: ignore
    // uri") and silently run tools against $HOME instead — and `tempfile`'s
    // default prefix is `.tmp`, so the default would have tripped it. That was
    // fixed upstream (re-probed @ 1.1.20: a turn in a hidden dir writes into
    // that dir). The explicit prefix stays: a real project dir is never hidden,
    // so this matches production either way, and it keeps the test from
    // depending on the fix holding.
