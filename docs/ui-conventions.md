...
- **Run state → `StatusDot`** (or the `status-*` tokens directly). Pass `StatusDot`'s `label` only when the dot is the *sole* signal (it then gets an accessible name + tooltip); omit it when a sibling text label already carries the meaning. It also accepts `status="warning"` for the caution role — an MCP server the harness reports as `needs-auth` is not "failed", it is something the user can fix. That value is deliberately **not** in `BadgeStatus`: that type is the run-state vocabulary, and `warning` is a separate token role so re-tuning run colors cannot drag warnings along.
- **Hover text → `Tooltip`**, never the browser's native `title` attribute. Use `SUPPLEMENTAL_TOOLTIP_DELAY` for low-priority details such as full truncated names, paths, timestamps, and status explanations so they do not appear during casual pointer movement. Set `focusable={false}` when the tooltip merely expands text already present in the DOM (especially inside another control), so it does not add a redundant Tab stop; keep focus enabled when the tooltip carries otherwise unavailable information. Dense repeated metadata such as transcript timestamps stays out of the Tab order even when its hover label uses a different format—expose the full value through accessible text instead. When supplemental details live inside an existing control, make that control the tooltip trigger rather than introducing a nested focus target.
