...
2. **Command line construction:**
   ...
   **Resume-flag verification probe.** M2.1 confirmed `codex exec resume <thread_id> "..."` works but did **not** verify the full flag set (`--json --skip-git-repo-check --dangerously-bypass-approvals-and-sandbox -C`) against the `resume` subcommand specifically. Before wiring resume into the adapter, run a one-shot probe on a real session id (NOT a sanitized fixture id — M2.1 fixture UUIDs like `00000000-0000-7000-8000-...` do not exist on disk): either pull a real session id from the developer's `~/.codex/sessions/` directory, or run a fresh `codex exec --json "ack"` to create a tiny session and use its thread_id. Then: `codex exec resume <real-session-id> --json --skip-git-repo-check --dangerously-bypass-approvals-and-sandbox -C /tmp "ack"`. Cross-check `codex exec resume --help` for flag-ordering constraints (some CLIs require options before/after subcommands). If any flag is rejected, document the difference in `docs/research/codex-cli-observed.md` and adjust the resume command-line shape before continuing.
