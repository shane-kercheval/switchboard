...
- **Live tests** (named `live_<harness>_…`): construct the **real adapter**, `dispatch` with an `AgentRecord` carrying a model (and, for Claude/Codex, an effort), and assert the values surface in the adapter's emitted `AdapterEvent` (`SessionMeta`/per-turn model; Codex `turn_context` effort) — driven through our code, **not** a bare CLI probe, so it covers both the CLI honoring the flags *and* our `build_args`. Tiny prompts per cost discipline.
...
- No value normalization/allow-list here; values pass through verbatim. (Model's empty-string footgun is handled at the form boundary in M5; effort is enum-constrained.)

### Definition of Done
- Tests on the free functions: create/attach with each field stores it; `set_agent_model`/`set_agent_effort` update and persist (reload proves durability); **clearing persists `None`**; setting a model on Antigravity or an effort on Gemini/Antigravity (all paths) returns the capability error **and leaves no orphan sidecar**; the attach rejection happens before any sidecar write.
- `make check` green.
...
- **Add `model: Option<String>` and `effort: Option<String>` to the `Turn` wire shape** (`crates/harness/src/transcript.rs`, and TS `LoadedTurn`/`Turn` in `src/lib/types.ts`), **separate from** the agent-scoped `SessionMeta` (which is a standalone, non-turn-anchored event, events.rs:108).
...
- **Antigravity model carry-forward:** Antigravity writes a `USER_SETTINGS_CHANGE` "from X to Y" sentence only on the turn where the model *changed* (verified: turn 1 `None → X`, and a resume after a settings change wrote `X → Y`). Maintain a running "current model": set it to the announced "to" value on a turn carrying a change, inherit otherwise, and stamp every turn. On **hydrate**, walk turns chronologically from the start; **live**, hold the running value in per-agent state (extend the existing "empty-model-keeps-prior" rule to stamp the turn). `None` only before any announcement (e.g. an attached conversation truncated before its first).
- **Live path correlation:** for Claude/Codex/Gemini the model arrives as an agent-scoped `SessionMeta` event with no turn anchor — associate the most-recent value(s) with the turn being finalized at `TurnEnd`/turn close. Transmit this explicitly; the model is not already turn-anchored. (Antigravity's carry-forward state is the analogous mechanism for that harness.)
- **`SessionMeta.model` consumer audit.** After M6 the sidebar shows `AgentRecord.model`, not `SessionMeta.model`. Check for remaining consumers; if none, remove the field for a clean architecture; if a harness's per-turn population reuses it as an intermediate, keep it only as that. Record the decision.
...
- **Live tests — the upstream-contract guard (required), all adapter-driven** (construct the real adapter, `dispatch`, assert on emitted `AdapterEvent` per-turn values — never grep raw CLI output; this catches both upstream drift *and* our build_args/parser/carry-forward regressions):
