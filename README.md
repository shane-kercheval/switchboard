...
- **Reasoning effort.** Every supported agent CLI lets Switchboard set the reasoning-effort level per agent (alongside the model). **Antigravity's current effort depends on the model you pick,** and some of its models have no effort setting at all. Quick choices for other models remain configurable, while the current-effort control appears only when the selected model can use it.
...
- **Codex models depend on your plan and CLI version.** When you sign in to Codex with a ChatGPT subscription, only the models your plan includes are available; choosing one your plan doesn't cover fails the turn with Codex's own error. GPT-6 Astra requires Codex 0.153.0 or newer.
...
- **Browser access is Claude Code only, and off until you turn it on.** With the [Claude in Chrome](https://chromewebstore.google.com/detail/claude/fcoeoabgfenejglbffodgkkbkcdhcgfn) extension installed, Settings → Browser access lets Claude agents open pages, read console errors, and fill forms in a visible Chrome window, using your existing logins. It needs a Claude Pro, Max, Team, or Enterprise plan. Switchboard has no way to check whether the extension is installed — if it isn't, agents run normally and just report that the browser is unavailable when they try to use it. **If more than one browser is connected to your Claude account, the first agent that needs the browser will ask which one to use; answer once and the choice sticks.** Codex's browser support is separate and is switched on in the ChatGPT desktop app, so this setting doesn't affect it.
...
- **Compacting a conversation is Claude Code only.** _Compact context_ asks Claude to summarize the conversation so far and carry on from that summary — how you reclaim a filling context window without starting a new agent. It queues like a message: if the agent is busy it runs after the work ahead of it, and Stop cancels it. Codex and Antigravity conversations can't be compacted from Switchboard; both accept `/compact` as ordinary text and then reply that they compacted while nothing actually happened, so Switchboard doesn't offer an action it can't really perform. Two things to expect after a Claude compaction: the row showing the before/after token counts lasts only for the current session — reopen the project and Claude's own summary marker is what remains — and if you do reopen, the sidebar's context gauge reads the pre-compaction number until that agent's next reply.
- **Seeing what's in a context window is Claude Code only.** The context gauge on a Claude agent's card has a chart icon that opens _Context breakdown_ — what is actually taking up the window, split by category and by item: the system prompt, tools, each MCP server's tools, custom agents, memory files, and skills. It is usually how you discover that most of a window went to tools nobody chose to load. It costs nothing and takes about a second, and it queues like a message if the agent is busy. Codex and Antigravity conversations can't be analyzed from Switchboard, for different reasons. Codex answers `/context` by making up a plausible-looking breakdown, and invented numbers that read like measurements are worse than no panel. Antigravity has no headless version of the command at all. The breakdown is a snapshot of one moment and every reply afterwards adds to the window, so the panel always says when it was taken — press Refresh for a current reading. A breakdown you run in the agent's own terminal won't appear here.
