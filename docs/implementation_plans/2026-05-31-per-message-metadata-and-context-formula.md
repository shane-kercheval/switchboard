...
- Update `contextUtilization` (`src/lib/components/Sidebar.svelte`) to sum all four token contributors over the denominator. **Note the current formula is `(input_tokens + output_tokens) / context_window` — it already excludes the captured `cached_input_tokens`, not just the dropped `cache_creation_input_tokens`.** So the frontend change is *two* additions (the existing cached-read field plus the newly-captured cache-creation field), not one. Keep the existing guard: if `context_window` is absent/zero, return `undefined` and the bar stays hidden (do not fabricate a denominator).
...
**Verify the stamp's ordering assumption before relying on it.** Stamping at turn-end assumes the dispatcher has already seen the turn's latest `rate_limit_event` by the time `result`/`TurnEnd` fires. For Claude this is the expected order (rate-limit streams mid-turn), but `events.rs` documents that for *Codex* the enrichment events (`RateLimitEvent`, `SessionMeta`) arrive **after** `TurnEnd` and before `AgentIdle`. Confirm against a real Claude stream that the rate-limit precedes `result`; if it can trail, the stamp must read the last-known snapshot up to `AgentIdle`, not only what arrived before `result` — otherwise the first overage turn is mislabeled "no overage."
