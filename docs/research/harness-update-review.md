...
|---|---|---|---|
| Codex | `0.139.0` | 2026-06-11 | changelog + code review (`0.137.0`→`0.139.0`) — no impact. `exec` flags, stream vocabulary (`thread.started`/`item.*`/`turn.completed`/`turn.failed`), `turn_context` fields (model/effort/turn_id), session-file path, `401 Unauthorized` auth classification, and exit-code semantics all unchanged. The 0.139 `resume`/`fork` arg-parse fix (#26818) targets the `--last` variant; our adapter resumes positionally via `exec resume … -- <thread_id> <prompt>` (unaffected). v2 personal-access-token auth (#25731) is additive — the `auth.json` presence probe is shape-agnostic and 401 classification is untouched. MAv2 / sandbox / plugin / TUI / Bedrock changes don't touch the exec contract |
