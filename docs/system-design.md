...
- **Fork** — create a new agent branched from the current state. Native in Claude Code via `--resume` + `--session-id` + `--fork-session` (the forked id is caller-controlled). Because a Claude branch can only come into existence *as a turn* (the CLI refuses a promptless fork — harness-behavior §3.5), Fork is a **send-time option in the compose bar** (a chip beside Forward), not an agent-row action: with a single Claude agent selected, enabling it makes that send create `<name>-fork`, place it in its own pane, move the compose selection to it, and dispatch the message as the branch's first turn. Available in plain compose mode (not while composing with a saved prompt — a tracked gap, not a boundary). Disabled — with an explanatory reason, announced on focus rather than hover-only — for non-Claude recipients, multi-recipient selections, an agent with no session yet, attached forward sources (Fork and Forward are both send modifiers and don't compose), or a parent that is mid-turn (a mid-turn snapshot would inherit a synthesized "No response requested." stub in place of the parent's in-flight answer). The mid-turn block is re-checked authoritatively when the send dispatches, but it is a look rather than a lock: a concurrent automation or an external CLI writing the same session can still slip through, yielding that stub rather than corruption. Not wired for the other harnesses in v1 (per resolved 10.14; per-harness ground truth in [harness-behavior.md](harness-behavior.md) §3.5).
