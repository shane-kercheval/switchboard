...
| Read rate-limit / quota | native (`rate_limit_event.unifiedWindows` on the stream — **undocumented**, found by probe, guarded by a live test) | native (`token_count.rate_limits` in session file) | available but **deliberately unbuilt** (`agy -p "/usage"` returns both windows for both model groups; costs seconds per turn — see harness-behavior.md §5.1) |
...
- **Read rate-limit / quota**: Claude and Codex, in different shapes. Codex enriches a `RateLimitEvent` from the session file's `token_count.rate_limits` post-terminal (class B, durable). Claude emits one on the stream carrying a `unifiedWindows` map — every key becomes its own meter — plus a second event when a window passes its warning threshold; that payload is stream-only, so it is persisted to the metadata sidecar and shown with a snapshot age after a restart. **Neither field is documented by its vendor**, and Claude's was found only by probing, so `live_claude_rate_limit_carries_unified_windows` exists to notice if it moves. Antigravity surfaces no comparable signal.
