...
4. **Probe Codex resume + session-file behavior (same-day; cross-day best-effort).** Run `codex exec ...` once; capture the session-file location. Then run `codex exec resume <id> ...`. Does Codex append to the original session file, or create a new file?
   ...
   **Cross-day handling.** macOS doesn't allow easy date-shifting without root, `faketime` isn't standard, and Docker is overkill. Capture a same-day resume fixture (works without time tricks); document the expected cross-day behavior from Codex source-of-truth notes; rely on M2.3/M2.4 unit tests with a **mocked sidecar `original_start_date_utc`** to prove the date-partition logic mechanically. Mark any cross-day live test `#[ignore = "requires cross-day window"]`. Full cross-day live verification is best-effort — manual two-day wait or future CI work — not blocking for M2.1.
...
6. **Verify Codex install method.** Codex CLI is not on Homebrew. Document under a `### Install path (candidate)` subsection in `docs/research/codex-cli-observed.md`. Deliverable shape: a single shell snippet a fresh macOS GitHub Actions runner can copy-paste (e.g., `npm install -g @openai/codex` or the appropriate published-release binary install), plus the expected post-install binary location (`which codex` output) and a `codex --version` smoke check confirming the install succeeded. Fresh-runner verification happens in M2.8.
...
> **⚠ Pre-implementation banner (post-M2.1).** The body of M2.3 below was written before the M2.1 probe pass ran. M2.1 surfaced direct contradictions that supersede the plan body:
> - **MCP tool calls flow as `mcp_tool_call`, NOT `command_execution`** — the M2.3 mapping table is updated in `docs/research/codex-cli-observed.md` §"Findings during M2.1". Implement both item types.
> - **Auth-failure detection is stream-based for both harnesses, not stderr-only** — see the per-harness stream patterns in the findings doc (Codex: `turn.failed.error.message` contains `"401 Unauthorized"`; Claude: `assistant.error == "authentication_failed"`). The stderr-and-exit-code rule in step 5 below is superseded.
> - **`turn.failed.error.message` is sometimes a JSON-encoded JSON object**, not a plain string — see the findings doc for the best-effort unwrap strategy before assigning to `TurnOutcome::Failed.message`.
>
> Before starting M2.3, re-read the "Findings during M2.1" section in `docs/research/codex-cli-observed.md` and apply the corrected mappings inline as you implement. The plan body below is authoritative on **intent and scope**; the findings doc is authoritative on **event-type mappings and detection patterns**.
...
- **Live integration test** (env-var-gated): `make test-live` runs `codex exec` for real; asserts a small response includes expected text.
...
> **⚠ Pre-implementation banner (post-M2.1).** The body of M2.4 below was written before the M2.1 probe pass. M2.1 confirmed two load-bearing assumptions and surfaced one clarification:
> - **`session_meta` is written once at line 1 and is NOT repeated on resume** (verified via `grep -c '"type":"session_meta"'` on a resumed session file → `1`). The "parse line 1 for `session_meta`" enrichment path is correct for both first turns and resumes.
> - **`token_count.rate_limits` and `task_started.model_context_window` are session-file-only** (not in the stream). M2.4 owns both `RateLimitEvent` emission for Codex and `context_window` population on `TurnEnd.usage`.
> - **Resume appends to the original spawn-date's session file**, so `original_start_date_utc` from the session-link sidecar must drive the date-partition lookup, never `Utc::today()`.
>
> Before starting M2.4, re-read the "Findings during M2.1" section in `docs/research/codex-cli-observed.md` and the sanitized `rate-limits.session.jsonl` fixture as the authoritative shape reference.
...
2. **`make test-live`** → all integration tests pass. Should complete in 1–3 minutes.
...
5. **Run the suite:** `make test-live`. Fails on any test failure.
...
5. **Local parity** — `make test-live` runs the same commands locally as CI.
