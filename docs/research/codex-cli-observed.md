...
- The Codex two-process model (Node parent + Rust child) was **not directly probed in M2.1** (no `ps -ef` capture, no signal-handling probe). M2.3's `process_group(0)` work assumes the tree exists per the archived `harness-comparison.md`'s prior observation. Process-group sanity is verified experimentally in M2.2 (Claude Code) and M2.3 (Codex) — neither M2.1 nor this doc certifies it independently.
