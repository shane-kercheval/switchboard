...
- **Rate-limit / quota signal**: harness-asymmetric per the v1 cost surface (§2). On the sidebar this is Claude's rate-limit window + reset and a current overage status, or Codex's quota % used — **not** a per-turn dollar figure. Per-turn cost is a *per-message* surface in the transcript (shown only on real-spend turns), not a sidebar cell; there is no per-agent cost total on the card. Per-harness availability — see "Per-harness sidebar surface" below. **No raw token counts are surfaced in the UI for any harness** — the underlying tokens are plumbed through the normalized event stream and used to compute context utilization, but the user-facing surface is dollars (Claude, per-message) or quota (Codex), not "X input tokens." See §9 for the normalized event vocabulary.
