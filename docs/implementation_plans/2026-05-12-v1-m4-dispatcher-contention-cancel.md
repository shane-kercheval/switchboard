...
**Resurrection seam — name the M4.4↔M4.6 ownership now (review).** M4.4's actor guarantees only **rejection-while-closing**: once `Shutdown` completes and the dispatcher drops the `AgentHandle`, a *new* `send_message` for that agent would spawn a **fresh** actor (with fresh context). What prevents resurrecting a torn-down project's agent is the **app-layer register-cache entry being gone** — which is **M4.6's** job, not M4.4's. So M4.6's remove-directory must remove the agent's register-cache entry **before (or atomically with)** issuing `Shutdown`; a racing send between handle-removal and cache-removal would otherwise spawn a zombie actor writing into a closed project. M4.4 ships the per-agent `Shutdown` primitive + its tests; the directory-batch teardown and the cache-removal ordering are M4.6. This is exactly the M4.4↔M4.6 seam where the original teardown race lived — named here so it is designed, not rediscovered as a bug.
