...
- Codex-specific quirks (SIGTERM-exits-0, two-process model, error event ordering, `AuthFailure` stream-based detection per M2.1 findings) all handled.
...
3. **Session-id handling** (Codex only). Per-agent session-link sidecar at `<directory>/.switchboard/projects/<project-id>/sessions/<agent_id>.jsonl`:
   ...
   - **On first stream event (`thread.started` with `thread_id`), append a new record to the sidecar IMMEDIATELY** (before any other parsing). Record shape (this is the M2.3→M2.4 contract — `original_start_date_utc` drives M2.4's date-partition path lookup, `session_id` drives the filename glob; do not rename or restructure these fields without coordinating M2.4 changes):
     ```json
...
4. **Stream parsing.** Read stdout line-by-line; parse each JSON line; map per the table:
   ...
   **`agent_message` granularity.** Codex emits `item.completed` for `agent_message` **without a prior `item.started`**. Tool items (`command_execution`, `mcp_tool_call`) emit both; text messages emit only the completion. Don't write a parser branch for an `item.started` of `agent_message` — it never arrives.
...
5. **Auth-failure detection (stream-based for both harnesses).** Per M2.1 findings, both Codex and Claude Code surface auth-failure as stream events, not exit-code-plus-stderr.
   ...
   - **Claude Code (state-flag pattern — DO NOT emit a second `TurnEnd`).** Claude's terminal event is `result`, which already owns the sole `TurnEnd` emission (`parse_result` at `crates/harness/src/parser.rs:139`). Auth-failure detection refines the `FailureKind`; it does **not** introduce a second emitter. The M1.3 one-terminal-event-per-turn contract must hold.
     - When `parse_assistant_envelope` sees an `assistant` event with top-level `"error": "authentication_failed"`, stash two things on `ParserState`: `auth_failure_seen: bool = true`, and `auth_failure_message: Option<String>` extracted from the assistant event's `message.content` array — specifically the first text content block's `text` field. If no text block is present, fall back to the raw `error` field value (`"authentication_failed"`).
     - When `parse_result` runs on the subsequent `result` line, everything it does today still runs (usage extraction included — auth-failure `result` events still carry a `usage` block, typically zero-valued; that's legitimate telemetry, not noise). The only behavior change: if `auth_failure_seen`, the emitted `TurnEnd` has `kind: AuthFailure` (with the stashed `auth_failure_message`) instead of `HarnessError`.
...
12. **Extend `SWITCHBOARD_HARNESS=mock` selection** to swap **both** adapters for `MockHarnessAdapter`:
...
13. **`AuthFailure` enum variant.** Add to `FailureKind`:
    ```rust
...
- **Fake-harness fixtures** for Codex: replay each M2.1 fixture through the parser and assert the expected `AdapterEvent` sequence.
  ...
  - **MCP tool failure path** (from M2.1's `mcp-tool-call.jsonl` fixture): ToolStarted with `name = "<server>.<tool>"` + ToolCompleted with `is_error: true` and the error text surfaced in `output`.
...
> **Note (post-M2.1).** M2.1 confirmed M2.4's load-bearing assumptions; the body below is correct as written. This note records the verification:
...
>
> The sanitized `rate-limits.session.jsonl` fixture in `crates/harness/tests/fixtures/codex/` is the authoritative shape reference.
