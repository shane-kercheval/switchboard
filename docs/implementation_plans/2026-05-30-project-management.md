...
- **Frontend.** `ProjectActionsMenu` shows Archive or Unarchive based on `project.archived`, **enabled regardless of availability** (and is now the reason the menu renders on unavailable — never-busy — rows). Relax the kebab gate from `available && !busy` to `!busy` (busy still hides the whole menu; see the gating decision above). `workspace.svelte.ts` `setProjectArchived(projectId, archived)` calls through, then flips `archived` on the matching `projects.list` row in place. `ProjectsSidebar` header gets a "Show archived" toggle (`$state` local); the rendered list is a `$derived` filter — exclude archived unless the toggle is on; archived rows get a muted treatment + the Unarchive affordance.
...
- Component tests: archived rows hidden by default; toggle reveals them de-emphasized; Archive/Unarchive item reflects current state and calls through; the menu (with Archive) appears on unavailable rows while Rename/Delete stay disabled there; the menu stays hidden on a busy row (the busy-hide is milestone-wide).
