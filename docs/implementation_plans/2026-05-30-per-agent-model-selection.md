...
- **Claude Sonnet reasoning live guard (carried over — depends on `--model` plumbing landing here).** Once Claude dispatches with a selected model, add a `live_claude_…` test that dispatches Sonnet (`--model sonnet`) on a non-trivial, thinking-inducing prompt and asserts a **non-empty** `ContentChunk { kind: Thinking }` arrives. This is the only automated guard that the real Sonnet CLI still returns *un-redacted* reasoning — Claude's redaction is per-model (`harness-behavior.md` §3.2), and until per-agent model selection exists there is no way to pin Sonnet in a live test (the dev default is Opus, which redacts), so today the contract is covered only by a parser unit test plus the mandated per-model re-probe on CLI bumps. If Sonnet silently re-redacts, nothing automated catches it until then. Assert `Thinking` *content presence*, not exact text; tiny prompt per cost discipline.
