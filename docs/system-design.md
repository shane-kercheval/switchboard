...
- **Switchboard owns the user's side** — a per-project append-only **conversation journal** (`projects/<project-id>/journal.jsonl`) recording each *send* (prompt + recipient + a `send_id` that groups a fan-out's recipients; written **when the turn starts**, one record per recipient) and each *cancelled-turn outcome* marker (outcome metadata only, never agent content). **Durable history begins when a turn starts, not when the user hits Send:** a message still queued behind a busy agent is live-UI-only and lost on restart (the queue is in-memory), and a queued message removed before it starts is never journaled. *Failed* turns are not journaled — they render from the harness file where the harness recorded them.
- **The harness session files own the agents' side** — all agent-produced content (responses, tool calls), read on hydration.

The unified transcript is the merge: user messages from the journal (grouped by `send_id`, rendered once — a fan-out is one "User → B|C" entry, not one per recipient), agent content from the harness files, cancelled turns from the journal's outcome markers. After restart a partially-queued fan-out shows only the recipients whose turns actually started (e.g. "User → A" if B was still queued) — intended, since B never received the message. A per-agent "what this agent saw" view can still read a single harness file verbatim (including the user message as that agent received it).
...
The user sees a **single unified transcript view** for the project — every turn from every agent appears in one chronologically-ordered stream, with each turn attributed to the agent that produced it (name + harness badge). Tool calls, errors, and per-turn metadata (cost / context utilization) nest under the turn they belong to. The user reads the project's conversation flow as one timeline; "Agent X responded at 10:02, then I forwarded to Agent Y at 10:03, then Y responded at 10:04" is legible at a glance without manually correlating across panes. The user's **own** messages appear **once** in this timeline regardless of how many agents received them — a single fan-out send to several agents is one "User → B|C" entry, not one per recipient (the user's side of the conversation is Switchboard-owned; see §3 and "Cancelling a workflow or turn" below).
...
The per-agent FIFO queue (manual sends) is the extent of queueing in v1. What is *not* in v1 is cross-agent **dependency chaining** outside a workflow — e.g. "send agent A a message built from agent B's not-yet-produced output." That is the job of the workflow engine's auto-forward / fan-in primitives (§4 Primitives 3–4); the compose-bar queue only orders *independent* messages to a single agent and never makes one message wait on another agent's response.
...
If Switchboard buffered streaming output from the cancelled turn (whatever the agent had produced before the kill), the user can review that partial content in-app — it's available from the buffered stream, not from the harness session file. The buffered partial *content* is in-memory only; restarting Switchboard discards it. The *fact* that the turn was cancelled, however, persists: cancelled turns are recorded as **outcome markers** in Switchboard's conversation journal (outcome + source + timestamp + agent/send, no content — see §3). On restart, history correctly shows the turn as cancelled rather than silently omitting it. (Failed turns are not journaled — they render from the harness file where the harness recorded them.) This follows the §3 split — Switchboard persists the user's sends and cancelled-turn *outcomes*; agent-produced *content* stays sourced from the harness session files, which remain the durable record of completed turns (and by design omit incomplete ones).
