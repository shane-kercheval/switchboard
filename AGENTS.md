...
- **Filesystem layout.** Per-directory Switchboard state lives at `<directory>/.switchboard/` — directly under the user's working directory, not in `~/.switchboard/`. `config.yaml` and `workflows/` are intended to be git-tracked; everything else is runtime data the user should `.gitignore` themselves. Local prompts are **user-global**, not directory-scoped — there is no per-directory `prompts/` store (system-design §6). User-global state lives in the OS-conventional config dir resolved via the `directories` crate (illustrative `~/.config/switchboard/` on Linux; `~/Library/Application Support/switchboard/` on macOS): personal `config.yaml` (prompt providers) and `prompts/` (default local prompt store), plus `workspace.yaml` — the app-managed set of working directories the user works across, the source for the flat cross-directory project list. See system-design §3 for the full layout.
