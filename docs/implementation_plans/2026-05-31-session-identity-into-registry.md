...
- **Capture persistence reuses the dispatcher's injected-sink pattern, but is load-bearing — not best-effort.** The dispatcher, on a capture event, persists the locator to the registry through an injected sink (parallel to `MetadataCache`/`ConversationJournal`, supplied by the app's `DispatchContext` factory). Unlike the metadata cache, a persist failure on **first capture** must fail the turn (synthesize `AdapterFailure`), preserving today's Codex behavior (a lost locator means the next turn starts a fresh session and silently drops context). Persistence happens **once** — on first capture; subsequent dispatches read the locator from the registry and never re-persist (today's per-dispatch re-append is dropped, with no functional loss).

- **Attach writes the registry inline; only first-dispatch capture uses the event path.** The attach commands run in the app and already hold registry access, so they write the locator directly into the registry record (no event, no sidecar). This also removes the current "sidecar-before-registry" ordering dance.
...
- Existing local dev registries still load (back-compat read), so the user does not lose already-created agents.

### Implementation Outline

- **Model.** Introduce `SessionLocator` (in core, beside `AgentRecord`). Replace `AgentRecord.session_id: Option<Uuid>` with the optional locator field (name the field for what it is now — it's no longer "just an id"). Claude registration produces `Some(Uuid::now_v7())`-shaped locator; Gemini `Some(Uuid::new_v4())` (**keep v4** — Gemini's filename prefix collision constraint is load-bearing and pinned by a test). Codex/Antigravity registration stays `None`.
- **Back-compat read (preserve existing dev agents).** Old records serialize `session_id` as a bare `Option<Uuid>`. Make deserialization tolerant: an old `session_id: <uuid>` maps to `SessionLocator::Uuid`, an old `null`/absent maps to `None`. (Codex/Antigravity old records were `None` and simply re-capture on next dispatch.) Implement via serde alias/`default`/custom deser — the agent picks the mechanism; the **contract** is "old registries load without error, Claude/Gemini agents keep their session." If the agent finds this materially harder than expected, stop and raise it — accepting a recreate-your-dev-agents breaking change is the fallback, but the shim is preferred.
...
- **Migrate Claude/Gemini reads.** Their `build_args` and attach/hydration read the locator from the `AgentRecord`; switch them from the old `session_id` field to matching the `SessionLocator::Uuid` variant. No logic change — same `--session-id`/`--resume` decisions.

### Definition of Done

- **Core tests:** locator round-trips through `registry.jsonl` for each variant; the update-in-place op replaces only the target agent's locator and preserves others (and order); back-compat deser maps an old `session_id: <uuid>` record to `SessionLocator::Uuid` and an old `null` to `None`. Corrupt-registry fail-loud behavior unchanged.
...
- **Docs:** record the locator model + the "identity → registry, nature-not-timing" rule in the `AgentRecord`/`SessionLocator` docstring. Note the back-compat decision (and its limit) in a comment.
- **Known limitation:** Codex/Antigravity still write sidecars after this milestone (removed in M2/M3) — the field exists but isn't yet populated at runtime for them. State this explicitly so the half-migrated state isn't mistaken for a bug.
...
- **Adapter tests (fixture-driven):** first Antigravity dispatch captures the conversation-id and emits the capture event with the right locator; a resume dispatch (locator already on the record) issues `--conversation <uuid>` and emits no capture; no `.antigravity.jsonl` is created.
- **Dispatcher tests:** a capture event invokes the sink once with the locator; a sink failure on first capture yields a failed turn (`AdapterFailure`); a non-failing capture persists and the turn proceeds.
...
## Milestone 4 — Migrate existing projects (lazy, on open) and delete the legacy code

### Goal & Outcome

Projects created before this refactor keep working with no manual steps and no lost session continuity. On project open, any legacy session-link sidecar is folded into the agent's registry locator and the file removed; legacy-shaped registry records are normalized. Once migration owns the legacy read path, the retained sidecar code is deleted.

This is **required, not cleanup**: after Milestones 2–3 the live adapters no longer read session-link sidecars, so an existing Codex/Antigravity agent whose locator lives only in a sidecar would otherwise silently start a fresh session and drop its context. It also doubles as the fix for the user's own existing dev projects — opening each one migrates it.

Outcomes:
- Opening a pre-refactor project transparently migrates its agents: Codex/Antigravity locators move from the sidecar into the registry record; the sidecar file is deleted; resume keeps working (same session, no lost context).
- Legacy `session_id`-shaped Claude/Gemini registry records are rewritten to the canonical locator shape.
- Migration is **idempotent** (re-open is a no-op) and **safe** (a corrupt/unreadable legacy sidecar is surfaced, never silently dropped — the file is left in place for inspection).
- After this milestone the legacy sidecar modules are gone; no session-link files remain anywhere.

### Implementation Outline

- **Lazy, on project open.** Hook migration into the existing open-project path (where the registry is loaded and `agents_by_id` is populated), under the `registry_write` lock. Per-project, per-agent — only projects the user actually opens are migrated; no eager full-workspace scan (a project you never open needs no migration until you open it).
- **Per agent:**
  - Codex agent with a legacy `<agent-id>.jsonl` and no registry locator → read the latest sidecar record (the migration-only reader retained in M3) → write a `Codex { thread_id, partition_date }` locator via the M1 update-in-place op → **only after that registry write is durable**, delete the sidecar file.
  - Antigravity agent with a legacy `<agent-id>.antigravity.jsonl` → same, writing a `SessionLocator::Uuid` → delete the file.
  - Claude/Gemini record still in the legacy `session_id` shape → normalize to the canonical locator shape (rewrite). The M1 tolerant-deser already lets it load; this makes the on-disk shape canonical so the shim can eventually be retired.
- **Ordering is load-bearing:** never delete a sidecar before its locator is durably in the registry. A failed registry write leaves the sidecar untouched (next open retries). An already-migrated agent (locator present, stale sidecar lingering from a prior failed delete) just gets the stale file deleted.
- **Corrupt legacy sidecar:** surface it (per the fail-loud-on-corrupt invariant); do **not** delete it and do **not** leave the agent locator-less such that it silently resumes fresh — the user must know that agent's session can't be recovered.
- **Delete the legacy code last.** Once migration owns the legacy read path, remove the retained sidecar modules/structs from M2/M3. **Keep** the M1 tolerant-deser for now — a registry in a never-opened project still needs it; note retiring it as future work once all known projects are confirmed migrated.

### Definition of Done

- **Tests:** opening a project with a legacy Codex sidecar migrates it (locator in registry, file deleted, resume args correct); same for Antigravity; a legacy `session_id` Claude/Gemini record normalizes; re-open is a no-op (idempotent); a simulated failed registry write leaves the sidecar in place (no data loss); a corrupt sidecar surfaces and is not deleted.
- **End-to-end (fixture-driven):** a migrated project resumes its Codex/Antigravity agents correctly with the locator now sourced from the registry (through the open→dispatch path).
- **Grep:** no session-link sidecar modules remain; `sessions/` holds only `.meta.json` (plus the metadata plan's `.turnmeta.jsonl`).
- **Docs:** system-design §10.3 + AGENTS.md record that session-link sidecars are fully removed and migration is automatic-on-open; note the **downgrade limitation** (a migrated project won't load on a pre-refactor build) and that retiring the tolerant-deser shim is future work.
- **Known limitation (record):** no downgrade support — once migrated forward, an older build can't read the project. Acceptable for a desktop upgrade path.
...
## Open questions for the human (resolve before the relevant milestone)

1. **M1 back-compat:** confirm preserving existing dev agents via a tolerant locator deserialization (recommended), versus accepting a one-time "recreate your dev agents" breaking change. The shim is cheap; the only reason to skip it is if it proves unexpectedly fiddly, in which case the agent will stop and ask.
