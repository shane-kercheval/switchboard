...
Switchboard stores all of its MCP prompt-provider credentials in one Keychain item. The first launch after upgrading from the older per-provider format may ask for access to each existing credential while it migrates them. After migration, an unchanged app restarts without another prompt; a newly rebuilt ad-hoc app asks once total after you choose **Always Allow**, regardless of provider count. Build-from-source approvals can recur after another rebuild until stable Developer ID signing is delivered.
