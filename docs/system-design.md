...
- **UI compose-bar dispatch:** a send to a busy agent is **queued**, not refused. Switchboard keeps a per-agent FIFO queue; when the agent's in-flight turn reaches a terminal state, the next queued message dispatches automatically. The queued item owns a transient snapshot of the model/effort profile active at submission; switching the agent afterward affects later sends, not work already in line. The queued message is visible inline ("queued — agent X is busy") and the user can remove it before it dispatches (its text returns to the compose bar per "Composing and dispatching messages"). The queue is **per-agent and in-memory** — queued-but-undispatched messages and their profile snapshots do not survive an app restart, consistent with in-flight turns (which also don't).
