...
| Select model per agent | native (`--model`) | native (`-m`) | native (`-m`) | **unavailable** (global config only) |
| Select reasoning effort per agent | native (`--effort`) | native (`-c model_reasoning_effort=`) | **unavailable** (config-only) | **unavailable** (folded into model name) |
