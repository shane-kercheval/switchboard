...
- `remove_agent_impl` — **two phases, and the split is mandatory, not stylistic.** `lock()` returns a `std::sync::Mutex` guard (held synchronously everywhere in `commands.rs`), but `dispatcher.shutdown_agent` is `async`. Holding a `std::sync` guard across an `.await` is a non-`Send` compile error under the multi-threaded Tokio runtime. So the shutdown **must not** happen inside the locked section.
  - **Cancel-and-remove, not reject-if-active (decided during M5 review).** The original draft said "reject if a turn is in flight," but the dispatcher exposes **no** non-destructive in-flight query (`Active` slot ≠ in-flight turn — an idle agent that has sent keeps an `Active` actor), so a backend reject would require building a busy-query + actor round-trip that still has a TOCTOU. Instead, `remove_agent` **unconditionally** `await`s `dispatcher.shutdown_agent(agent_id, CancelSource::Shutdown)` — which cancels any in-flight turn (surfaced as a `Cancelled` terminal, same machinery as "Stop"), drains the harness subprocess, and drops the actor slot. "Can't remove while active" is a **UI affordance** (M6's disabled button); the backend's job is to be robust regardless of state, which `shutdown_agent` already provides. Resolve the agent's project via `lookup_agent` (agent-id → own project), so these ops are free of the active-project coupling M4 flagged. Inherits `shutdown_agent`'s drain latency — note it in a comment.
  - **Phase (a) — no lock held:** `await dispatcher.shutdown_agent(...)`.
...
- App tests (free-function `*_impl` level, per the crate's convention): remove deletes present sidecars and tolerates absent ones; remove updates `agents_by_id`; remove of an agent with an **in-flight turn cancels it (turn journaled `Cancelled`) and removes cleanly** (the cancel-and-remove contract); remove of a non-existent id errors; rename updates `agents_by_id` and returns the record; rename to a duplicate leaves the cache untouched.
...
**Tracked follow-up (out of M5 scope):** `io::write_yaml` has the same pre-`write_jsonl` durability gap (no `sync_data` before rename, no parent-dir fsync after) — it persists `workspace.yaml` (the user's cross-directory project list) and `config.yaml`, so it's worth hardening the way `write_jsonl` now is. Noted in `io.rs`; not done here.
