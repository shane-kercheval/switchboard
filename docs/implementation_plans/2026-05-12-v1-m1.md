...
- **M4** adds multi-pane UI, dispatcher contention enforcement, and per-turn cancel.
- **M5** adds prompt providers (slash commands).
- **M6+** add workflows, pause-for-user, iteration, and the rest of v1.
...
1. Read these files first (in order):
   ...
   - `docs/implementation_plans/2026-05-12-v1.md` — the M1 section in particular, plus the "Critical path" preamble.
...
2. Implement sub-milestones M1.1 → M1.5 in order. Each sub-milestone is self-contained: code + tests + doc updates. Stop after each one, summarize what landed, and wait for human review before continuing. The whole M1 branch ships as a single PR after M1.5 — do not open per-sub-milestone PRs.
...
The M1 acceptance from `docs/implementation_plans/2026-05-12-v1.md`:
...
Do not consider M1 done until **all five sub-milestones are complete and reviewed** and this end-to-end flow works on a clean macOS checkout. The whole M1 branch ships as a single PR after M1.5.
...
- Per-turn cancellation (M4)
- Project-level `flock` for multi-instance protection (M4)
- Slash commands / prompt providers (M5)
- Workflows (M6)
- First-launch acknowledgement dialog (M8)
- Tray, walk-away, signing, auto-updater (M8)
...
1. **Initialize Tauri 2.x app** using the official template. Reference: <https://tauri.app/start/create-project/>.
   ...
   - **Version pinning policy:** use caret ranges (`^x.y.z`) in `package.json` and bare version strings in `Cargo.toml` (Cargo's default caret-equivalent semantics), and **commit both `pnpm-lock.yaml` and `Cargo.lock`**. CI uses `pnpm install --frozen-lockfile` and `cargo build --locked` for byte-identical reproducibility. The lockfile is the source of truth for resolved versions; manifests document the supported range. This is the standard pattern for binary applications in both ecosystems and is what `bits-ui`/`shadcn-svelte`/Svelte 5 peer-dep resolution expects — hard-pinning with `=` causes resolution failures when transitive deps need overlapping-but-not-identical ranges. (This deviates from earlier drafts of the plan that called for hard pins; the lockfile alone provides reproducibility, and hard-pinning manifests adds zero value on top of that.)
...
3. **Project hygiene + agent context.** Files that aren't code but make the repo navigable for humans and AI agents alike:
   ...
   - **`AGENTS.md`** — playbook for AI agents (and humans) working on the codebase. Living doc, extended each sub-milestone. Initial sections:
     ...
     - **Architecture overview** (Rust workspace layout, Svelte frontend, Tauri shell — pointer to `docs/implementation_plans/2026-05-12-v1.md` for milestone roadmap)
     ...
     - **Authoritative docs** (`docs/system-design.md`, `docs/implementation_plans/2026-05-12-v1.md`, the per-milestone plans under `docs/implementation_plans/`, the research notes under `docs/research/`)
...
7. **README.md update.** Add a "Local development" section. Don't duplicate Makefile commands; reference them. Roughly: "Prerequisites: Node + pnpm (see `.nvmrc`), Rust (see `rust-toolchain.toml`), Xcode CLT (macOS). Common commands: `make dev`, `make test`, `make check`. See `AGENTS.md` for project context, `docs/implementation_plans/2026-05-12-v1.md` for the milestone roadmap."
...
Hand off to the human for review. M1 ships as a single PR after M1.5 — no per-sub-milestone PR.
...
- Both the **agent registry** and the **project index** hold **N entries from day one** (forward-compatible with the project switcher UI in M4 and the agent selector UI in M2). M1.5's UI exposes one active project displaying one agent — switchers come in later milestones.
...
- **User-global config (OS-conventional path via the `directories` crate; for `local_prompt_dirs` and MCP provider configs per `system-design.md` §6) is M5+ scope — M1 does not read or write it.** M1 has no prompts (M5) and no MCP (M5), so nothing in M1's scope materializes the user-global layer. The `directories` crate dependency, the schema, the override-vs-fallback resolution logic, and the tests all land alongside prompt providers in M5. Implementing agent: do not add user-global config handling to M1; the silence is intentional.
...
- `docs/implementation_plans/2026-05-12-v1.md`'s deferred-detail callout ("Persistence schema details (10.3)") — note that M1 lands the registry + project-index shapes; the runs/checkpoint shape is still M6.
...
1. **Normalized event types.** Two enums: `AdapterEvent` (what adapters emit) and `NormalizedEvent` (what crosses IPC). The split makes "TurnStart is dispatcher-owned, never adapter-emitted" a type-level invariant rather than a doc convention.
   ...
   pub enum TurnOutcome {
       ...
       // Future: Cancelled { source: CancelSource } — added in M4 when per-turn
   ...
   **Why `kind` on `Failed` (not just `message`):** the discriminator is load-bearing for M6's partial-failure rule (the human-in-the-loop pause we filed in `docs/implementation_plans/2026-05-12-v1.md` M6). When one agent in a fan-in fails, the workflow needs to distinguish "harness reported a model error" (`HarnessError` — bad prompt, retry won't help) from "subprocess crashed" (`AdapterFailure` — transient, retry might help). The UI can use the same discriminator to surface different affordances ("retry" vs "this is a Switchboard problem, restart the agent"). Internal Rust code at the adapter layer can carry richer cause detail in logs without surfacing it on the wire.
...
2. **`HarnessAdapter` trait.** The minimum surface for M1:
   ...
   pub trait HarnessAdapter: Send + Sync {
       async fn dispatch(
           ...
           project_root: &Path,
           prompt: &str,
           turn_id: TurnId,           // dispatcher-generated; adapter embeds in every emitted AdapterEvent
...
3. **`ClaudeCodeAdapter`.** Constructs the command line:
   ...
   claude -p \
     ...
     --session-id <uuid>    # first turn (no session file on disk yet)
     # OR --resume <uuid>   # subsequent turns (session file exists)
   ```
   **`--session-id` idempotency probe result (M1.3):** `--session-id` is NOT idempotent — passing it for a UUID that already has a session file creates a new session instead of resuming. `--resume` is required on subsequent turns. The adapter checks `~/.claude/projects/<encoded-cwd>/<uuid>.jsonl` before each dispatch: if the file exists → `--resume`; if not → `--session-id`. See edge cases in the M1.3 section below.

   **Do not add a mutable `initialized: bool` field to `AgentRecord`.** Two reasons: (a) it violates the registry's append-only invariant; (b) ground truth is more robust — if the user manually deletes a session file, the FS check sees "no file" and the adapter creates a fresh session via `--session-id`, but a registry flag would still say `initialized: true` and the adapter would call `--resume`, which Claude rejects (file's gone), leaving the agent stuck.
...
6. **Subprocess working directory.** `Command::current_dir(cwd)` where `cwd` is the user's **bound working directory** (i.e., `Project.directory`, the folder the user opened — _not_ `Project.root`, the internal `.switchboard/projects/<uuid>/` metadata directory). Claude needs to see the user's repo files via its Read/Glob/Bash tools — that's the entire M1 use case. The plan originally said "bound project" which was ambiguous and got implemented wrong during M1.5; surfaced when the cwd bug was reviewed post-implementation. Session-file storage follows the same cwd: `~/.claude/projects/<encoded-working-dir>/<session-uuid>.jsonl`. Multiple projects in the same working directory share that storage directory; per-agent sessions are distinguished by session UUID.

7. **Error handling.** Distinguish two cases:
   - **`claude` binary not found on PATH** — at adapter construction (or first dispatch), use `which::which("claude")` to detect. Return a typed `BinaryNotFound` error from the adapter constructor — *not* via `TurnEnd(Failed)`. This surfaces as an app-level banner (M1.5), not a per-turn failure. Minimum-version assertion is deferred to M8.
...
9. **`MockHarnessAdapter`** — second implementation of `HarnessAdapter` that produces canned events programmatically (no subprocess at all). Purpose: **dev-time iteration on the M1.5 UI without `claude` installed or authenticated**, plus useful as a deterministic test harness for the dispatcher beyond what `RecordingEmitter` covers (which only tests emission, not adapter behavior).
   ...
   pub enum MockScenario {
       Streaming,   // default — text streaming + clean TurnEnd(Completed)
       Panic,       // adapter task panics mid-stream — used by M1.4's panic test
       // Future: ErrorMode, Latency, etc. Add as the test suite needs them.
   }
   ...
   impl MockHarnessAdapter {
       pub fn new() -> Self { Self::with_scenario(MockScenario::Streaming) }
       pub fn with_scenario(scenario: MockScenario) -> Self { Self { scenario } }
   }

   #[async_trait]
   impl HarnessAdapter for MockHarnessAdapter {
       async fn dispatch(&self, _agent: &AgentRecord, _project_root: &Path, prompt: &str, turn_id: TurnId)
           -> Result<EventStream, DispatchError>
       {
           // For MockScenario::Streaming, construct an EventStream that yields:
           //   ContentChunk { turn_id, kind: Text, text: "Mock response to: " }
           //   ContentChunk { turn_id, kind: Text, text: <prompt> }
           //   ContentChunk { turn_id, kind: Text, text: " — replied by mock harness." }
           //   TurnEnd { turn_id, outcome: Completed, ended_at: <now> }
           //
           // (M1's TurnEnd has no usage field; M2.2 adds usage and the mock will
           //  emit Some(TurnUsage { ... }) at that point.)
           //
           // Do NOT add artificial delays between chunks — they add test overhead with no
           // assertion value. For MockScenario::Panic, panic!() inside the stream task at the second yield.
           //
           // turn_id is provided by the dispatcher (per M1.4 step 5 ordering).
           ...
       }
   }
   ```
   ...
   **Selection at runtime.** App startup reads `SWITCHBOARD_HARNESS` env var:
   ...
   - `mock` → construct as `MockHarnessAdapter::new()` (Streaming scenario; no `claude` lookup, no `which::which("claude")` check, no `BinaryNotFound` banner).
   - Any other value → panic with a clear error listing valid values. (Don't silently fall back to default; that's a footgun.)

   The selection lives in `crates/app`'s startup code where `AppState` is constructed (see M1.4 step 3). M2 extends this to also swap the Codex adapter (per M2.3 step 8).

   **Scope limits for M1:**
   - Two scenarios in the `MockScenario` enum: `Streaming` (default for dev iteration) and `Panic` (used by M1.4's dispatcher panic test). Future milestones add `ErrorMode`, `Latency`, etc. as the test suite needs them.
...
```rust
use tokio::io::{AsyncBufReadExt, BufReader};   // tokio::io::BufReader, NOT std::io::BufReader
...
- **Live integration test.** Gated behind `#[ignore]`. Run with `make test-live`. Runs `claude -p` for real, asserts that "What's 2+2?" produces some `ContentChunk` containing `"4"` and a `TurnEnd(Completed)`. Run this manually before merging M1.3 — do not enable in CI for M1 (M2 stands up the integration CI workflow).
- **Negative tests.**
...
- **`MockHarnessAdapter` test.** Drain the stream from `MockHarnessAdapter::with_scenario(MockScenario::Streaming).dispatch(...)`; assert the canned sequence (`ContentChunk`s × N → `TurnEnd(Completed)`). Verifies the trait is satisfied correctly and the dispatcher (M1.4) can be tested end-to-end against it without spawning real `claude`. Also test `MockScenario::Panic` (used by the M1.4 panic test): the dispatch task panics mid-stream → the dispatcher's `AgentIdleGuard` Drop restores Idle. Expand scenarios as the test suite needs them.
...
- **`AGENTS.md`** — add the exactly-one-terminal-event stream contract, the AdapterEvent vs NormalizedEvent split rationale (TurnStart is dispatcher-owned, never adapter-emitted), the parser-precedence rule (deltas authoritative when `--include-partial-messages` enabled), the subprocess-lifecycle pattern (`Stdio::null()` at spawn, drain stderr concurrently, `child.wait()` after terminal event), the FailureKind discriminator on TurnEnd, the **`MockHarnessAdapter` for dev-time UI iteration without real `claude`** (selected via `SWITCHBOARD_HARNESS=mock` env var), and the **`tokio::io::BufReader` gotcha** ("for async subprocess stdout, use `tokio::io::BufReader`, not `std::io::BufReader` — `tokio::process::ChildStdout` doesn't implement `std::io::Read`").
...
2. **`--session-id` idempotency — already resolved (M1.3).** `--session-id` is NOT idempotent; the FS-check branch was taken. No re-probe needed.
3. **Live integration test** — `make test-live` runs the real-`claude` smoke tests. Should complete in ~15–20s (two tests). Expect at least one `ContentChunk` containing `"4"` and a clean `TurnEnd(Completed)` for `live_basic_turn_completes`; `live_session_continuity_across_turns` verifies `--resume` works on the second turn.
...
### Edge cases discovered during M1.3 implementation

These surfaced during implementation and live testing. The decisions live in the v1 roadmap (`2026-05-12-v1.md`) under the milestone that owns the relevant area, so they're not lost when M1 ships:

- **Session file deleted externally**, **`CLAUDE_CONFIG_DIR` / non-standard storage**, and **`--session-id` vs `--resume` undocumented coupling** — all three are Claude Code session-file resilience concerns. Filed under **M2 scope** ("Decisions deferred from M1.3 — Claude Code session-file resilience"), where the harness-adapter abstraction gets its first hardening pass.
- **Concurrent Switchboard instances sharing a directory** — race on `session_file_exists()` for the same agent UUID. Mitigated by M4's `instance.lock`; noted there.
...
- App state holds **one bound working directory** and **N projects under it** (multi-project from day 1) per system-design §3 + M1.2; the M1 UI exposes one **active project** at a time (project switcher lands in M4). Multi-directory is not in scope for v1.
...
This sub-milestone establishes the chokepoint pattern that M4 will harden into the formal "single dispatcher" with contention enforcement. For M1, it's just the entry point shape — concurrency hardening is M4.

### Implementation outline

1. **`Dispatcher`.** Owns `HashMap<AgentId, AgentState>` behind a `std::sync::Mutex` (sync mutex; state ops never `.await` while held — required for `AgentIdleGuard::Drop` to work, see step 6). `AgentState` for M1 is just `{ status: Idle | InFlight }` — enough to refuse a `send_message` if the agent already has a turn in flight. **This is a minimal local guardrail, not the M4 chokepoint.** Return a generic "agent is busy" error string. M4 owns the actual error taxonomy (typed errors, structured contention reasons, UI gating treatment) — do not preemptively model those in M1.

   **Dispatcher API (pin this — gives the implementer a precise target and makes the testing-strategy "free function" instruction unambiguous):**
   ```rust
   impl Dispatcher {
       /// Dispatch a turn to the given agent. Returns synchronously after step (e)
       /// of M1.4 step 5: AgentIdleGuard acquired, TurnId generated, adapter.dispatch()
       /// called, TurnStart emitted on success, drain task spawned. Returns the TurnId.
   ...
   ```
   The `send_message` Tauri command is a thin shim over a free function `send_message_impl(state: &AppState, agent_id: AgentId, prompt: &str) -> Result<TurnId, DispatchError>` that does the AgentRecord lookup (per step 4 below) then calls `dispatcher.send_message(...)`. Test the free function directly — the testing-strategy target.
...
3. **App state.** A `tauri::State<AppState>` shape (multi-project from day 1):
   ...
   ```
   The M1.5 UI picks one `active_project_id`; project switcher (UI for changing it) lands in M4. Background activity for non-active projects keeps running because the dispatcher is global and event channels are agent-scoped — see step 5.
...
4. **Tauri commands.** Working-directory + project lifecycle commands take or return path / id strings; per-agent commands take `agent_id` (no `project_id` needed since agent_ids are globally unique and the dispatcher routes via `AgentRecord.project_id`).
   ...
   ```
   Returning `String` for the error type is a Tauri convention; map `thiserror`-typed errors to `to_string()` at the boundary. **`send_message` returns the `TurnId` synchronously** — the dispatcher generates it BEFORE calling `adapter.dispatch()` (so the adapter can embed it in every emitted `AdapterEvent`), passes it as a parameter to `dispatch()`, and on success emits `TurnStart` with the same `TurnId`. See step 5 for the full ordering.

   **AgentRecord lookup for `send_message`** — deterministic: scan `AppState.projects` for a project whose `registry.jsonl` contains the requested `agent_id`. If found, dispatch with that project's root + the agent's harness type. If the agent is not in any loaded project, return a typed `AgentNotFound { agent_id }` error to the caller. **No implicit "active project" routing** — the agent_id is globally unique (UUID v7), and routing is by lookup, not by UI state. For M1, scanning all loaded projects' registries on each `send_message` is fine (registries are small, M1 has one loaded project). An in-memory `HashMap<AgentId, (ProjectId, AgentRecord)>` cache on `AppState` is a reasonable M4+ optimization if send rate becomes a concern in workflow contexts.
5. **Turn lifecycle and event forwarding.** The ordering here is load-bearing — it satisfies the round-2 invariant (pre-stream `DispatchError` returns synchronously, never leaving an orphan `TurnStart` on the wire), the concurrent-send-race protection (two simultaneous `send_message` calls for the same agent: only one passes the Idle check), AND the TurnId-embedding invariant (every `AdapterEvent` from the stream carries the same `turn_id`, so the dispatcher must generate it BEFORE calling `adapter.dispatch()` and pass it as a parameter). The `AgentIdleGuard` and the `turn_id`-as-parameter together enforce all three.
   ...
   - **(b)** Generate fresh `TurnId` (UUID v7).
   - **(c)** Call `adapter.dispatch(..., turn_id)` — the `TurnId` is passed in so the adapter can embed it in every emitted `AdapterEvent`. **The state lock is not held during this `.await`** — concurrent `send_message` calls for the same agent see `InFlight` and return `Err(Busy)`.
   ...
   - **(d)** Emit `TurnStart` via `EventEmitter` with the same `turn_id` from step (b) — this is the first normalized event on the per-agent channel for this turn, so the frontend can show "processing" before any harness-emitted event is consumed from the stream. (The harness subprocess is already running by this point — `adapter.dispatch()` spawned it in step (c) — but no `AdapterEvent` has been forwarded yet.)
   - **(e)** Spawn the drain task with ownership transferred (via `move`) of the `EventStream`, the `AgentIdleGuard`, and an `Arc<dyn EventEmitter>`. Return `TurnId` to the caller synchronously.
   ...
   - **Two-state model: `AgentState { status: Idle | InFlight }`.** `InFlight` covers both "reserved for dispatch" (after the `AgentIdleGuard` is acquired but before `adapter.dispatch()` returns) and "actively running a turn" (after the stream is established). Concurrent sends in either window return `Err(Busy)` to the caller — the user-facing error message is the same; M1 doesn't need a finer state distinction. (M4 may add structured contention reasons.)
   ...
   - **Backpressure: M1 emits each event naively, one Tauri event per `NormalizedEvent`.** This will not scale to M4's multi-pane fan-out (one fan-out turn × N agents × hundreds of token deltas). M4 expansion must address this — design space includes the §10 ring buffer, coalescing windows, rate limiting, or size caps. See the deferred-from-M1 callout in `docs/implementation_plans/2026-05-12-v1.md` M4.
   - **Background activity for non-active projects keeps running.** The dispatcher is global; agent IDs are globally unique; event channels are per-agent. Switching the active project in the UI (M4) is purely a display change — agents in other projects keep streaming, workflows in other projects keep running, events keep firing on their per-agent channels. The frontend reducer routes events to the right agent's transcript regardless of which project is currently active. **Project unload is not a v1 concept** — once a project is opened (`open_project` / `create_project`), it stays in `AppState.projects` for the lifetime of the app session. There is no `close_project` command. Memory grows with N projects × M agents × K turns; bounded for v1, may need LRU eviction or per-project hibernation in v2+. **M1 carve-out:** in M1 only one project is ever loaded at a time (the one the user just opened/created). The multi-project subscription model is *available* via the AppState shape but not *exercised* until M4's project switcher actually loads multiple projects. Don't over-build M1's subscription path against multi-project scenarios that won't fire — M4 puts the property under real load.
...
- **Dispatcher unit tests** (no Tauri — use `RecordingEmitter` for assertions).
  ...
  - **Panic test:** a panicking dispatch task does not leave the agent stuck `InFlight` — `AgentIdleGuard`'s `Drop` impl restores state. Use `MockHarnessAdapter::with_scenario(MockScenario::Panic)` (per M1.3 step 9) to validate.
...
- **`AGENTS.md`** — add the dispatcher-is-the-single-chokepoint rule, the `EventEmitter` trait + `RecordingEmitter` testing pattern, the `AgentIdleGuard` RAII pattern (with `std::sync::Mutex` rather than `tokio::sync::Mutex`), the dispatch ordering (acquire `AgentIdleGuard` first → state `Idle → InFlight` under lock → release lock → **generate fresh `TurnId`** → call `adapter.dispatch(..., turn_id)` with the turn_id passed in so the adapter can embed it in every emitted `AdapterEvent` → `DispatchError` causes guard to drop on early return restoring `Idle` → `TurnStart` fires *only after* `dispatch()` returns `Ok` with the same `turn_id` so no orphan `TurnStart` ever lands on the wire), the per-agent event channel `agent:<id>` convention, and the multi-project AppState shape (one bound directory at a time, N projects loaded, one active project for UI display, dispatcher is global because agent IDs are globally unique). Also: **TurnId is dispatcher-owned and passed as a parameter to `adapter.dispatch()`**; adapters never generate TurnIds.
...
2. **Tauri command free-function tests** — assert the free functions backing each Tauri command (`init_directory`, `list_projects`, `create_project`, `open_project`, `set_active_project`, `create_agent`, `list_agents`, `send_message`, `check_claude_binary`) behave correctly when invoked directly with a constructed `AppState`. Don't try to test the `#[tauri::command]` wrapper itself.
...
2. **Open WebView devtools** and exercise the command surface. Suggested sequence:
   ...
   ```
   Expect to see in the console: `turn_start` event → multiple `content_chunk` events with text → `turn_end` with `outcome: { status: 'completed' }`. (No `usage` field on `turn_end` in M1 — that lands in M2.2 as a wire-format addition.) Each event payload's `turn_id` matches the one returned synchronously.
...
- One **active project** displayed in the pane. The directory may have multiple projects (M1.2 supports this), but the M1 UI shows one project's pane at a time. **Project switcher UI lands in M4.**
...
8. **Terminal-event timeout / stream-coherence guard.** The dispatcher (M1.4) does not synthesize a fallback `TurnEnd` if an adapter violates the stream contract — stream-contract ownership stays with the adapter (M1.3) on purpose. The frontend reducer must therefore handle the failure mode itself: after receiving `turn_start`, if no terminal event (`turn_end`) arrives within a bounded window (suggested: 60s with no `content_chunk` activity, i.e., a heartbeat-based timeout rather than an absolute wall-clock cap — long completions are legitimate), transition the agent's status to "error" with a "no response from harness — retry?" affordance and a CTA to dispatch a fresh turn. This is the M1.5 reducer's resilience against any future adapter bug that silently truncates a stream. Out of scope for M1: backend cleanup of the orphaned dispatcher state (the agent will be `Idle` per `AgentIdleGuard` Drop, so a retry just dispatches a new turn — no special server-side recovery needed).
...
Hand off to the human for review.
