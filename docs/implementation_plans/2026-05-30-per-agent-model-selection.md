...
### Implementation Outline
- Add "Change model" and "Change effort" actions for a capable agent, each gated on its capability map (so an Antigravity agent shows neither; a Gemini agent shows only "Change model"). **There is no `AgentActionsMenu.svelte`** — the first draft assumed a component that was never built. Today the per-agent actions (Stop / Resume in terminal / Open session file / Delete) live **inline in `Sidebar.svelte`** (~lines 654-772), and rename is a double-click inline edit, not a menu item. **Implementer decision (the plan doesn't pre-make it):** build a small dropdown from the existing `ui/DropdownMenu` + `ui/DropdownMenuItem` primitives and host the new items there (recommended — keeps a growing action set out of the inline bar), *or* add them to Sidebar's inline action row. Either way, the earlier "mirror the rename action" guidance does **not** apply (rename isn't menu-based). Each item opens an editor reusing the M5 picker, pre-filled with the current value; submit calls `setAgentModel(agentId, model?)` / `setAgentEffort(agentId, effort?)`.
