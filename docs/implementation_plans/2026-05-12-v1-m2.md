...
- Multi-pane UI (M4) — M2's UI adds the agent selector, but only one pane is visible at a time.
- Project-level `flock` for multi-instance protection (M4).
- Slash commands / prompt providers (M5).
- Workflows (M6).
- Workflow-level cancellation (M6).
- Pause-for-user / iteration (M7).
- First-launch acknowledgement dialog, tray, walk-away, signing, auto-updater (M8).
- Codex fork (deferred to v2+ per resolved 10.14 — Claude Code agents will get a Fork action in M4; Codex agents show an explanatory tooltip).
- ~~Cost reporting in any form.~~ **No longer a non-goal — open design decision for M2 expansion (see below).** The original v1 stance (flat non-goal, "subscription billing means no meaningful cost per turn") held when both harnesses metered subscription usage as opaque rate-limited consumption. Anthropic's 2026-06-15 split of `claude -p` onto a dollar-denominated Agent SDK credit pool broke that assumption — `claude -p` now emits `total_cost_usd` against a real depleting budget. Codex's model didn't change. M2 captures token counts in `TurnEnd.usage` regardless; the design decision below resolves whether/how to surface dollars or credits on top of tokens.
...
## Open design decision — per-agent cost / credit / quota surface

Originally a v1 non-goal (see system-design.md §3); reopened after Anthropic's 2026-06-15 Agent SDK credit-pool split. M2 expansion must resolve this before the M2.2 event vocabulary is finalized (so the data model knows what to plumb through `TurnUsage`).

**The asymmetry.** Each harness × auth-method combination has a different meaningful "consumption" surface:

| | ChatGPT/Claude subscription | API-key auth |
|---|---|---|
| **Claude Code (`claude -p`)** | Dollars consumed from finite monthly Agent SDK credit pool (Pro: $20/mo, distinct from interactive). `total_cost_usd` native in `result` event. | Dollars consumed (pay-as-you-go). `total_cost_usd` native. |
| **Codex (`codex exec`)** | % of rolling 5-hour window quota consumed (no dollar mapping). Tokens available; cost is NOT exposed natively. | Dollars consumed (pay-as-you-go). Cost must be derived from tokens × per-model pricing table. |

Unifying these as "cost" misleads three of the four cells. Unifying as "tokens" is technically uniform but throws away the most actionable signal in the Claude-subscription cell.

**Sub-decisions M2 expansion must resolve:**

1. **What to surface per cell.** Show dollars where natively available (Claude Code both auth modes)? Show "% of window" or "messages remaining" for Codex subscription? Show derived dollars for Codex API-key (requires shipping and maintaining a per-model pricing table per `docs/research/harness-comparison.md`)? Or pick a lowest-common-denominator metric (tokens-only) and let users compute dollars themselves?
2. **Auth-method detection.** Switchboard would need to know which auth mode each agent is using to pick the right surface. Claude Code: derivable from presence of `total_cost_usd`. Codex: harder — requires inspecting which auth file the user has, or asking at agent-creation time.
3. **Aggregation scope.** Per-turn only? Per-agent cumulative? Per-project cumulative? Per-session-since-app-launch?
4. **Pricing-table maintenance burden.** If Codex API-key cost derivation is in scope, who keeps the per-model pricing table accurate? OpenAI's model pricing changes regularly. Stale rates mean wrong numbers shown to users. Defer to v2 if this burden is too high.
5. **Implementation location.** Display work could land in M2 (extends the existing M1.5 single-pane) or M4 (when multi-agent UI lands). Data plumbing must be M2 regardless.
6. **Relationship to M8 onboarding.** Whatever's shown in-app needs an onboarding explanation of which credit pool / quota gets consumed and the post-2026-06-15 Claude Code split. M8 already has scope for this — keep them coherent.

Defer to v2 is a valid outcome for any sub-decision (e.g., "M2 plumbs `total_cost_usd` through `TurnUsage` but only displays it for Claude Code; everything else is v2"). What's not valid is unanswered.
...
- M1.5's TS types are extended to include the new variants. The reducer's default branch (M1.5 testing #4) keeps the UI from crashing; the new events are accepted without yet rendering anything new (rendering polish is M4+).

### Implementation outline

1. **Extend `AdapterEvent` and `NormalizedEvent`** with the four new variants AND two field additions on existing variants. M2.2 is the "pay the wire-breaks once" milestone — bundling the field additions here means M4+ doesn't pay them later when the reducer has more logic to update.
...
3. **Process-group spawn.** Replace `Command::new("claude")...spawn()` with `Command::new("claude")...process_group(0).spawn()` (Tokio's `Command` exposes this on Unix). This puts the harness in its own process group, so M4's `killpg` will reach the whole tree. M2 doesn't add the `killpg` call — that's M4.
...
6. **Cancellation detection (foundational for M4).** Per `codex-cli-observed.md` §"Cancellation": Codex's parent exits 0 on SIGTERM, so exit code alone doesn't distinguish "killed" from "completed." For M2, the adapter just needs to handle "stdout EOF without a `turn.completed` or `turn.failed` event" — synthesize `TurnEnd { outcome: Failed { kind: AdapterFailure, message: "stream ended without terminal event" } }` per the M1.3 stream-contract rule.
...
- Per-agent runtime metadata (model, tokens, context utilization, rate limits) is captured into project-level state and available for future UI surfacing (M2 doesn't yet render it; M4+ does).
- The `create_agent` flow accepts a harness type (currently hardcoded in M1).

This is **not multi-pane** — one pane visible at a time. Multi-pane is M4. **Project switcher is also M4.** M2's selector switches agents within the active project; switching the active project itself comes in M4.
...
2. **Per-agent runtime metadata also lives at the project level** — alongside transcripts but in a separate state map. Pin the shape now to prevent implementer-time improvisation:
   ...
   type AgentRuntime = {
     ...
     rateLimit?: unknown;               // harness-specific shape, surfaced opaque (UI rendering is M4+).
...
3. **Subscriptions persist for ALL loaded projects, for the lifetime of the app session**, with explicit lifecycle. Project unload is **not a v1 concept** — once a project is opened, it stays in `AppState.projects` until app exit so background activity (workflows in M6+, in-flight turns) keeps streaming into transcripts even when the user switches away. Lifecycle:
   ...
   **Why this model:** matches M6's concurrent-workflow narrative — workflows in non-active projects keep running and updating their UI state in the background. The alternative (tear down listeners on project change) would silently freeze background project state when the user navigates away, breaking M6. Memory grows with N projects × M agents × K turns of transcript history; for v1 this is bounded (no realistic v1 user has 50 projects × 1000 turns each), but **it's a known commitment to revisit in v2+** if scale needs LRU eviction or per-project hibernation.
