...
## Deferred from M2 — Codex session-file date-partition contract

**Surfaced while running `make test-live` against `db257d5`.** The Codex live test `live_codex_basic_turn_completes` fails on this branch *and* on baseline; it is not from this milestone's work, but it is real.

**Symptom.** `TurnEnd.usage.context_window` comes back `None` for Codex turns. `RateLimitEvent` and `SessionMeta` may also be empty depending on the user's timezone and the time of day.

**Root cause.** Codex partitions its session files by **local date**, not UTC. Observed:

- `date -u` → `Sun May 17 01:15:44 UTC 2026`
- `date` → `Sat May 16 18:15:44 PDT 2026`
- Session file written at that moment: `~/.codex/sessions/2026/05/16/rollout-2026-05-16T18-11-24-<uuid>.jsonl` — the **local** date in the directory path and the **local** timestamp in the filename.

Switchboard's adapter writes the sidecar with `original_start_date_utc = Utc::now().date_naive()` (`crates/harness/src/codex/mod.rs::try_persist_sidecar`) and `locate_session_file` searches `~/.codex/sessions/<that-date>/`. When local and UTC dates differ — any user west of UTC after ~16:00 local, or east of UTC before ~08:00 — the lookup misses, the retry loop misses, and the adapter returns `Enrichment::default()` with all fields `None`.

The parser is fine — handed the file directly, it correctly extracts `context_window=Some(258400)`. The bug is in the *locator*.

**Affected scope:**

- M2.4 enrichment for any timezone-day-boundary user.
- M2.5 attach-existing-session flow (the field is named `original_start_date_utc` and is parsed back out of the path on attach via `find_codex_session_file_for_attach` — same wrong-UTC assumption).
- The `SessionLinkRecord` schema field `original_start_date_utc`.

**Fix scope:**

- `crates/harness/src/codex/mod.rs::try_persist_sidecar` — capture local date, not UTC.
- `crates/harness/src/codex/session_file.rs::find_codex_session_file_for_attach` — parse local date from path.
- Sidecar schema: rename `original_start_date_utc` (the name bakes the wrong assumption into the contract; either rename to `original_start_date_local` or redefine semantics).
- All fixture tests + research doc (`docs/research/codex-cli-observed.md`) need to record the local-date partition behavior.
- Live test that exercises the date-boundary case (mock the clock, or run twice across a boundary).

**Open question for the fix pass.** Was Codex always local-date partitioned, or did it change? The M2.1 captured fixtures may answer this — if those were written assuming UTC, we got it wrong from the start; if they were local-date and we mis-labeled, the rename is mechanical.

A `TODO(date-partition)` marker at `try_persist_sidecar` points at this section.
