...
- **Fork session** — create a new agent branched from the current state. Native in Claude Code via `--resume` + `--session-id` + `--fork-session` (the forked id is caller-controlled). Not wired for the other harnesses in v1 (per resolved 10.14; per-harness ground truth in [harness-behavior.md](harness-behavior.md) §3.5) — the action is enabled only on Claude Code agents; the others see an explanatory tooltip.
