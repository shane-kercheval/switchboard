...
- **View-archived = an `Active | Archived` segmented toggle** (the existing `segmentedControl` component — the same two-pill control as the New project / Add existing dialog — sized down for the Projects header, placed left of the `+`). It's a true either/or filter: **Active** (default) lists non-archived projects; **Archived** lists only archived ones, each with an Unarchive action in its kebab. This supersedes the earlier "single list + 'Show archived' toggle with dimmed fold-in" idea (the engineer chose the cleaner either/or split: separated views need no dimming, and a project meant for continuous use is simply unarchived). Still not a 3-way `Active/Archived/All` control, and not a separate route/tab — one list whose contents the toggle filters. Sorting is unchanged (`last_activity` desc) within whichever view is shown.
- **Per-project menu (`ProjectActionsMenu`) gating — busy, then availability.**
  - **Busy hides the whole menu, in every milestone.** A project with in-flight turns shows only its progress spinner / cancel control (which owns the right slot); no mutation affordances are offered mid-run. Mutating a running project is intentionally unsupported in the UI — stop it (or let it finish) first. This is a product rule, not a backend constraint (rename/archive are both safe against a running project), chosen for a simpler, clearer running-project UI. It costs nothing reachability-wise: `unavailable ⇒ not busy`, so the busy-hide never strands the archive-an-unavailable-row case (below).
  ...
  - **Trigger renders only when ≥1 action is enabled.** M1–M2: `available && !busy` (the only actions, Rename/Delete, both need availability). M3 relaxes to `!busy` — Archive makes the menu meaningful on unavailable rows too (Rename/Delete disabled there). Status spinner/checkmark stays always-visible as the rightmost element; the kebab sits to its left, hover/focus/menu-open-revealed.
...
- **Frontend.** `ProjectActionsMenu` shows Archive or Unarchive based on `project.archived`, **enabled regardless of availability** (and is now the reason the menu renders on unavailable — never-busy — rows). Relax the kebab gate from `available && !busy` to `!busy` (busy still hides the whole menu; see the gating decision above). `workspace.svelte.ts` `setProjectArchived(projectId, archived)` calls through, then flips `archived` on the matching `projects.list` row in place. `ProjectsSidebar` header gets an `Active | Archived` segmented toggle (`$state` local view, default `Active`), reusing `segmentedControl` sized for the header, left of the `+`; the rendered list is a `$derived` filter on `project.archived` matching the selected view. Archived rows need no dimming (the view is all-archived); the kebab carries the Unarchive action. Also: hard-**delete** must scrub the project id from the global archived set (a future re-created id must not inherit a stale flag) — wire this into the M2 delete path now that the set exists.
...
- Component tests: archived projects hidden in the default `Active` view; switching to `Archived` shows only them; Archive/Unarchive item reflects current state and calls through; the menu (with Archive) appears on unavailable rows while Rename/Delete stay disabled there; the menu stays hidden on a busy row (the busy-hide is milestone-wide).
