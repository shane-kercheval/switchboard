...
7. **Lookup-strategy mechanics** for the date-partitioned path. If a session was started just before midnight UTC and the turn completes after, the session file is in the previous day's directory. Adapter uses the sidecar's `original_start_date_utc`, not the current date.
...
**Reducer scan complexity on long hydrated transcripts.** Pass A's `transcriptReducer` (`src/lib/state/reducers.ts`) does O(n) `findTurn` + O(n) `map` per event. For typical live sessions (<100 turns) this is fine. M2.6's hydration may produce **1000+ turn transcripts** on project open; a Claude streaming response into a hydrated transcript would then be O(n) per chunk × hundreds of chunks = **O(n²) for that turn**. The streaming UI would noticeably stall.

Mitigation if observed: refactor `TranscriptMap` from `Record<AgentId, Turn[]>` to `Record<AgentId, { order: TurnId[]; by_id: Map<TurnId, Turn> }>`. `findTurn` becomes O(1); `updateTurn` becomes O(1). Order-by-`started_at` for the unified-view merge happens at render time over the `order` arrays. This is a localized refactor (only `reducers.ts` + the unified-view selector), but it touches the state shape so M2.6 is the right place to land it if hydration surfaces the issue.

Decision deferred until M2.6 has real hydrated transcripts to test against — Pass A doesn't introduce the latency, and an early refactor risks over-engineering for a profile we haven't taken.
...
1. **Per-harness session-file parsers.** Free functions (not a trait — formats are too different to usefully abstract, and the mock adapter has no session file to mock):
   ...
   pub fn load_codex_transcript(session_id: Uuid, original_start_date_utc: NaiveDate, cwd: &Path) -> Result<LoadedTranscript, LoadTranscriptError>;
