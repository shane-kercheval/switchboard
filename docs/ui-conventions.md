...
- **Run state → `StatusDot`** (or the `status-*` tokens directly). Pass `StatusDot`'s `label` only when the dot is the *sole* signal (it then gets an accessible name + tooltip); omit it when a sibling text label already carries the meaning. It also accepts `status="success"` for a healthy state and `status="warning"` for the caution role. These values are deliberately **not** in `BadgeStatus`: that type is the run-state vocabulary, while environment health is a separate UI role. Warning also keeps its own semantic token so re-tuning run colors cannot alter caution states. Set `focusable={false}` when a labelled dot sits inside another focus-managed surface; its accessible name and hover tooltip remain available without adding a Tab stop.
- **Hover text → `Tooltip`**, never the browser's native `title` attribute. Use `SUPPLEMENTAL_TOOLTIP_DELAY` for low-priority details such as full truncated names, paths, timestamps, and status explanations so they do not appear during casual pointer movement. Tooltips open for pointer hover and keyboard-visible focus; focus restored by dialogs, mouse clicks, or application code stays quiet by default. Set `focusable={false}` when the tooltip merely expands text already present in the DOM (especially inside another control), so it does not add a redundant Tab stop; keep focus enabled when the tooltip carries otherwise unavailable information. Dense repeated metadata such as transcript timestamps stays out of the Tab order even when its hover label uses a different format—expose the full value through accessible text instead. When supplemental details live inside an existing control, make that control the tooltip trigger rather than introducing a nested focus target.
- **Structured click-open details → `Popover`**. Use it when the content has sections, scrolling, nested disclosure, or information the user may need time to inspect. Tooltips remain for short supplemental explanations; they must not be the only place critical state is available. A popover trigger is an explicit button with a useful accessible name, and its content uses a bounded height so long inventories remain reachable.
- **Hover-revealed row controls collapse to zero width; they do not sit as an invisible gutter.** `visibility: hidden` still charges the row for the control's width, and on a narrow surface that reserved gutter is taken straight out of the row's most-read text — on the agent card it held 59px beside a name clipped by 39px (measured in WebKit). Collapse with `w-0 overflow-hidden opacity-0 pointer-events-none` and restore width, margin and opacity on `group-hover` / `group-focus-visible` / `group-has-[:focus-visible]` / `group-has-[[data-state=open]]`, transitioning `[width,margin,opacity]` so revealing them slides neighbours rather than snapping them. Put the margin on the control's revealed state, not a `gap` on the container: a flex gap is charged for a zero-width child and leaves the gutter behind. A control that also encodes state (a visibility toggle on an already-hidden agent, a grip mid-drag) stays at full width regardless. Spell the variants out literally — Tailwind scans for whole class strings, so a composed `${variant}:w-[26px]` is never generated. `Sidebar.svelte`'s card header and `TranscriptPanes.svelte`'s pane-member chips are the two references.
- **Menus → `DropdownMenu` + `DropdownMenuItem`** (wraps `bits-ui`, so focus/keyboard/escape/ARIA come for free). Never hand-roll a menu.
- **A menu branch → `DropdownMenuSub`**. It is the standard for hierarchical menu choices — reach for it over an in-place accordion when the nested content is a list whose length you don't control, since each level is its own menu (arrow keys, typeahead, and escape scope to the level the user is in) and the parent doesn't grow a scrollbar because one branch was expanded. An accordion is still the better fit for a short, fixed set the user should see in context. `side` picks which way it opens, but only visually: bits-ui ties the open/close keys to text direction, so a left-opening submenu still opens on `→`. All three share `menuStyles.ts` — a menu surface and a menu row have one definition each.
  - **Constraint on async branches.** Children mount only while open, so `onOpenChange` is where you'd fetch what the branch lists — but bits-ui hands keyboard focus to the first item once, at open. Contents that arrive later are reachable by mouse and **not by keyboard** (`ForwardSourcePicker`'s project rosters have this today, knowingly). Either populate the branch synchronously or plan for a focus handoff after the data lands.
...
- **A "how full is this" gauge → `Meter`**. Label left, optional detail plus the used percentage right, a track-and-fill bar beneath. Every usage gauge goes through it — the context meter, each rate-limit window, and every category row in the context-breakdown panel — so the geometry and the percentage rounding are defined once. `value` is always the fraction **used**, 0–1: a source that reports remaining is converted where it is derived, never at the primitive. It owns no tooltip (call sites disagree on whether they want one), and a non-finite `value` renders nothing rather than a full bar reading "NaN%".
...
- **On/off toggle → `Switch`** (`role="switch"`). Controlled: the parent owns the boolean and flips it in `onclick`; the primitive owns the track/knob geometry and the disabled treatment. Takes `checked`, `disabled`, `ariaLabel`, `testid`, `onclick` — never hand-roll the track/knob markup.
