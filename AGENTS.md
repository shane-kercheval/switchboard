...
- **Harness-asymmetry: session-id ownership.** Claude Code pre-generates `AgentRecord.session_id` at agent registration (UUID v7) and passes it to `claude` via `--session-id` / `--resume`. Codex assigns its own session id (the `thread_id` from `thread.started`) on first dispatch; `AgentRecord.session_id` stays `None` for Codex agents for their lifetime, and the per-agent session-link sidecar at `<directory>/.switchboard/projects/<project-id>/sessions/<agent_id>.jsonl` is the system-of-record for the captured thread_id plus its `original_start_date_utc` (load-bearing for date-partitioned session-file lookup; never recomputed from `Utc::today()` on resume — Codex appends to the original spawn-date's session file).
- **Companion asymmetry: Codex-only `needs_session_meta` flag.** The Codex adapter gates `SessionMeta` emission on `prior.is_none()`. Attached agents satisfy `prior.is_some()` on first dispatch, so without an override the sidebar's MCP/skills/model listing stays empty until something else triggers emission. `AppState.needs_session_meta` (Codex-only insert in `attach_agent_impl`) forces `DispatchOptions::is_first_dispatch_after_attach: true` on every dispatch until a `session_meta` event is observed on the per-agent channel — a per-dispatch emitter decorator (`crates/app/src/emitter.rs`) clears the flag at that moment. Read-don't-drain semantics in `send_message_impl` mean repeated failures before SessionMeta is observed each continue to force it. Claude emits `SessionMeta` from its `system/init` stream event on every dispatch and needs no flag.
- **Codex post-terminal enrichment.** Codex's stream omits several fields (model context window, rate limits, model, harness version); the adapter reads the session file after each terminal stream event to fill them in. Mechanics in `crates/harness/src/codex/session_file.rs`.
- **AppState named-fields convention.** Per-harness adapters are exposed as named fields (`claude_adapter`, `codex_adapter`) on `AppState`, not as a single `adapter` field. `send_message_impl` selects via `match agent.harness`. Future harnesses are additive variants on `HarnessKind` plus a new named field on `AppState`. The `App routing test` in `crates/app/src/commands.rs` pins the routing against regression — a regression that hard-codes one adapter would silently spawn the wrong binary, invisible at the dispatcher layer (which is keyed by `AgentId` alone).
